 ...
 */
export function resolveSpreadsheetId(input: string | undefined | null): string {
  const raw = String(input ?? "").trim();
  if (!raw) {
    throw new GsheetsError(
      "invalid_argument",
      "spreadsheet_id is required.",
      "Pass the long id from the middle of the sheet's URL, or paste the whole URL and it will be read out of it.",
    );
  }

  if (ID_PATTERN.test(raw) && !looksLikeUrl(raw)) return raw;
  ...
  if (looksLikeUrl(raw)) {
    throw new GsheetsError(
      "invalid_argument",
      `No spreadsheet id in "${truncate(raw)}".`,
      "A Google Sheets URL looks like https://docs.google.com/spreadsheets/d/<id>/edit. This one has no /spreadsheets/d/ part, so it may be a Drive folder or a different kind of document.",
    );
  }

  throw new GsheetsError(
    "invalid_argument",
    `"${truncate(raw)}" is not a spreadsheet id.`,
    "An id is the long run of letters, digits, hyphens and underscores in the middle of the sheet's URL. A full URL works too. A tab name goes in the sheet argument instead.",
  );
}
...
export const SPREADSHEET_ID_DESCRIPTION =
  "The spreadsheet id, the long id in the middle of the sheet's URL. A full URL works too and the id is read out of it.";
