...
export const l04MergeInData: LintRule = {
  id: "L04",
  severity: "error",
  ...
  run(ctx: SheetLintContext): LintFinding[] {
    ...
    const tables = (ctx.sheet.tables ?? [])
      ...
      .filter((entry): entry is { table: NonNullable<(typeof entry)["table"]>; range: GridRangeLike } =>
        Boolean(entry.range),
      );
    ...
    for (const merge of merges) {
      ...
      if (!rangesOverlap(merge, dataBox)) continue;

      const crossesHeader =
        region.headerRow !== undefined &&
      ...
      findings.push({
        ...
        severity: "error",
        location: where,
        message: `${where} is merged inside the data on ${ctx.title} (${lintLocation(
          ctx.title,
          boundsA1(dataBox.startRowIndex ?? 0, dataBox.endRowIndex ?? 1, dataBox.startColumnIndex ?? 0, dataBox.endColumnIndex ?? 1),
        )}). ${reason}`,
