 ...
 */
export function tallyFormulaErrors(ctx: SheetLintContext): FormulaErrorTally {
  const tally: FormulaErrorTally = { totalFormulas: 0, errors: [], loading: 0 };

  eachCell(ctx.sheet, ({ row, column, cell }) => {
    if (typeof cell.userEnteredValue?.formulaValue === "string") tally.totalFormulas += 1;
  ...
  if (ctx.formulas) {
    ...
    for (const row of ctx.formulas) {
      for (const value of row ?? []) {
        if (typeof value === "string" && value.startsWith("=")) counted += 1;
      }
    }
    if (counted > tally.totalFormulas) tally.totalFormulas = counted;
  }

  return tally;
}

export const l01FormulaError: LintRule = {
  id: "L01",
  severity: "error",
  ...
  run(ctx: SheetLintContext): LintFinding[] {
    ...
    const findings: LintFinding[] = shown.map((cell) => ({
      rule: "L01",
...
function errorName(type: string): string {
  switch (type) {
    case "REF":
      return "#REF!";
    case "NAME":
