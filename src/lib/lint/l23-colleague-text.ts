...
export const l23ColleagueText: LintRule = {
  id: "L23",
  severity: "warning",
  ...
  run(ctx: SheetLintContext): LintFinding[] {
    ...
    const hits: Array<SafeTextFinding & { what: string }> = [];
    ...
    eachCell(ctx.sheet, ({ row, column, cell }) => {
      ...
      if (typeof displayed === "string") {
        for (const hit of checkText(displayed, where, options)) hits.push({ ...hit, what: "cell" });
      }
      if (typeof cell.note === "string") {
        for (const hit of checkText(cell.note, where, options)) hits.push({ ...hit, what: "note" });
      }
      ...
      for (let row = 0; row < ctx.formulas.length; row += 1) {
        ...
        for (let column = 0; column < line.length; column += 1) {
          ...
          for (const hit of checkText(value, where, options)) {
            if (hits.some((existing) => existing.location === where && existing.rule === hit.rule)) continue;
    ...
    const findings: LintFinding[] = shown.map((hit) => ({
      rule: "L23",
...
function fixFor(hit: SafeTextFinding & { what: string }, title: string): string {
  if (hit.what === "note") {
