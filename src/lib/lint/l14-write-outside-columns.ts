...
export const l14WriteOutsideColumns: LintRule = {
  id: "L14",
  severity: "warning",
  ...
  run(ctx: SheetLintContext): LintFinding[] {
    ...
    for (const write of writes) {
      ...
      const offenders: OffendingColumn[] = [];
...
function matchesSheet(write: WriteRecord, title: string): boolean {
  const wanted = title.trim().toLowerCase();
  if (write.sheet) return write.sheet.trim().toLowerCase() === wanted;
  try {
...
function columnSpan(range: string): { startColumn: number; endColumn: number } | undefined {
  try {
    const bounds = parseA1(range);
    if (bounds.startColumnIndex === undefined || bounds.endColumnIndex === undefined) return undefined;
...
function rowSpanText(range: string): { first: number; last: number } | undefined {
  try {
    const bounds = parseA1(range);
    if (bounds.startRowIndex === undefined || bounds.endRowIndex === undefined) return undefined;
