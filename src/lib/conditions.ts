...
export const CONDITION_KINDS = ["number", "text", "date"] as const;
...
const FIXED_KIND: Partial<Record<ConditionOperator, ConditionKind>> = {
  contains: "text",
  not_contains: "text",
  starts_with: "text",
  ends_with: "text",
...
const BY_KIND: Record<ConditionKind, Partial<Record<ConditionOperator, string>>> = {
  number: {
...
const RELATIVE_SET = new Set<string>(RELATIVE_DATES);

function conditionValue(raw: string | number | boolean, kind: ConditionKind): {
...
function needsValue(spec: ConditionSpec, which: "value" | "value2"): string | number | boolean {
  const raw = spec[which];
  if (raw === undefined || raw === null || raw === "") {
    throw new GsheetsError(
      "invalid_argument",
      `The ${spec.operator} condition needs ${which === "value" ? "a value" : "a second value"}.`,
      which === "value2"
...
export function resolveKind(spec: ConditionSpec): ConditionKind {
  ...
  if (typeof sample === "string") {
    if (sample.trim() !== "" && Number.isFinite(Number(sample))) return "number";
    if (RELATIVE_SET.has(sample.trim().toLowerCase())) return "date";
    if (/^\d{4}-\d{2}-\d{2}$/.test(sample.trim())) return "date";
    return "text";
  }
  return "number";
}
...
export function buildCondition(spec: ConditionSpec): BooleanCondition {
  const operator = spec.operator;

  if (operator === "blank") return { type: "BLANK" };
  if (operator === "not_blank") return { type: "NOT_BLANK" };
  ...
  if (operator === "custom_formula") {
    ...
    if (!formula) {
      throw new GsheetsError(
        "invalid_argument",
        "A custom_formula condition needs a formula.",
        'Pass formula, for example formula: "=AND($D2>0, $E2=\\"Confirmed\\")". Write it as if for the first cell of the range; Sheets fills the rest down.',
      );
    }
    return {
      type: "CUSTOM_FORMULA",
  ...
  if (operator === "one_of_list") {
    ...
    if (values.length === 0) {
      throw new GsheetsError(
        "invalid_argument",
        "A one_of_list condition needs the list of options.",
        'Pass values, for example values: ["Confirmed", "Pending", "Declined"].',
      );
    }
    return {
      type: "ONE_OF_LIST",
  ...
  if (operator === "one_of_range") {
    ...
    if (!range) {
      throw new GsheetsError(
        "invalid_argument",
        "A one_of_range condition needs the range the options come from.",
        'Pass source_range with the tab name, for example source_range: "Lists!A2:A20".',
      );
    }
    return {
      type: "ONE_OF_RANGE",
  ...
  if (!type) {
    ...
    throw new GsheetsError(
      "invalid_argument",
      `The operator ${operator} does not apply to a ${kind}.`,
      `On a ${kind} the operators are: ${usable}.`,
    );
  }
...
export function describeCondition(condition: BooleanCondition | undefined): string {
  if (!condition?.type) return "no condition";
