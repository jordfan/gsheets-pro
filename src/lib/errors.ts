...
export const err = {
  badRange: (range: string, why?: string) =>
    new GsheetsError(
      "bad_range",
      why ?? `Could not parse the range "${range}".`,
      "Ranges are A1: a cell like B2, a block like A1:C10, a whole column like B:B, or a whole row like 3:3. A tab name goes in the sheet argument, not the range.",
    ),
  sheetNotFound: (name: string, available: string[]) =>
    new GsheetsError(
      "sheet_not_found",
      `No tab named "${name}" in this spreadsheet.`,
      available.length
        ? `The tabs are: ${available.join(", ")}. Tab names are matched without regard to case.`
        : "This spreadsheet has no readable tabs. Check that the account can open it.",
      { available },
    ),
  invalid: (message: string, hint?: string) =>
    new GsheetsError("invalid_argument", message, hint),
  authMissing: (message: string, hint?: string) =>
    new GsheetsError(
      "auth_missing",
      message,
      hint ??
  ...
  tooLarge: (message: string, hint?: string) =>
    new GsheetsError("result_too_large", message, hint),
  internal: (message: string) => new GsheetsError("internal", message),
};
...
export function apiErrorMessage(error: unknown): string {
  if (error instanceof GsheetsError) return error.message;
  if (error instanceof Error && !(error as ApiErrorish).response) return error.message;
...
export function toStructuredError(error: unknown): StructuredError {
  if (error instanceof GsheetsError) return error.toStructured();
...
export function errorToText(e: StructuredError): string {
