 ...
 */
export function splitSheetRange(input: string): SplitRange {
  ...
  if (s.startsWith("'")) {
    ...
    if (rest === "") return { sheet: name, range: "" };
    if (!rest.startsWith("!")) {
...
export function toA1Reference(sheet: string, range?: string): string {
...
function parseCellRef(part: string, whole: string): CellRef {
  const s = part.trim().replace(/\$/g, "");
  const m = /^([A-Za-z]{1,3})?([0-9]+)?$/.exec(s);
  if (!s || !m || (m[1] === undefined && m[2] === undefined)) {
    throw new Error(
      `Could not parse A1 range "${whole}". Expected something like A1, A1:C10, B:B, or 3:3.`,
    );
  }
  const ref: CellRef = {};
  if (m[1] !== undefined) ref.col = columnLetterToIndex(m[1]);
  if (m[2] !== undefined) {
 ...
 */
export function parseA1(range: string): RangeBounds {
  if (typeof range !== "string") {
    throw new Error(`Could not parse A1 range: ${JSON.stringify(range)}`);
  }
  const body = stripSheetPrefix(range).trim();
  if (!body) throw new Error("Range is empty. Expected something like A1:C10.");
  ...
  if (cols.length) {
    ...
    if (cols.length === 2) {
      ...
      if (parts.length === 1) bounds.endColumnIndex = start.col + 1;
    } else {
      bounds.endColumnIndex = (end.col as number) + 1;
    }
  }
  ...
  if (rows.length) {
    if (rows.length === 2) {
      ...
      if (parts.length === 1) bounds.endRowIndex = start.row + 1;
    } else {
      bounds.endRowIndex = (end.row as number) + 1;
    }
  }
...
export function a1ToGridRange(range: string | undefined | null, sheetId: number): GridRange {
...
export function gridRangeToA1(range: NullableBounds): string {
  ...
  if (!hasCols && !hasRows) return "the whole sheet";
  ...
  if (hasCols && hasRows && startCell && startCell === endCell) return startCell;
  return `${startCell}:${endCell}`;
}

/** How many cells a range covers, or undefined when an axis is unbounded. */
export function rangeCellCount(range: NullableBounds): number | undefined {
  const startRowIndex = range.startRowIndex ?? undefined;
  const endRowIndex = range.endRowIndex ?? undefined;
  const startColumnIndex = range.startColumnIndex ?? undefined;
  const endColumnIndex = range.endColumnIndex ?? undefined;
 ...
 */
export function unionBounds(a: NullableBounds, b: NullableBounds): RangeBounds {
  const pick = (
    x: number | null | undefined,
    y: number | null | undefined,
    fn: (p: number, q: number) => number,
  ): number | undefined => {
  ...
  if (sr !== undefined) out.startRowIndex = sr;
  if (er !== undefined) out.endRowIndex = er;
  if (sc !== undefined) out.startColumnIndex = sc;
  if (ec !== undefined) out.endColumnIndex = ec;
  return out;
}
...
/** "A" -> 0..1, "B:D" -> 1..4. */
export function parseColumnSpan(spec: string): Span {
...
/** "1" -> 0..1, "2:5" -> 1..5. */
export function parseRowSpan(spec: string): Span {
  const body = stripSheetPrefix(String(spec ?? "")).trim().replace(/\$/g, "");
  ...
  if (a < 1 || b < 1) throw new Error(`Row numbers in "${spec}" start at 1.`);
