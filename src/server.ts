...
import { getContext, type Context } from "./lib/client.js";
...
export function createServer(options: CreateServerOptions = {}): McpServer {
  const deps: ToolDeps = { getContext: options.getContext ?? (() => getContext()) };
...
export function toolNames(): string[] {
  return createTools({ getContext: async () => ({}) as Context }).map((t) => t.name);
}
