 ...
 */
import { z } from "zod";
import { a1ToGridRange, gridRangeToA1, parseA1, parseColumnSpan, parseRowSpan, toA1Reference, } from "../lib/a1.js";
import { runBatchUpdate, withRetry } from "../lib/batch.js";
import { parseColorStyle } from "../lib/colors.js";
import { METADATA_KEYS, toMetadataEntries } from "../lib/contract.js";
...
import { compileTheme, loadPreset, manifestValue, resolveArchetype, resolvePresetName, roleFormat, ROLE_TOKENS, } from "../lib/theme.js";
const BORDER_STYLES = ["SOLID", "SOLID_MEDIUM", "SOLID_THICK", "DASHED", "DOTTED", "DOUBLE", "NONE"];
const BORDER_EDGES = ["top", "bottom", "left", "right", "inner_horizontal", "inner_vertical"];
const STYLE_MASK = [
...
export const styleInputSchema = {
    ...
    preset: z
        .string()
        .optional()
        .describe("The palette. With no range this writes the spreadsheet's theme, all nine slots, and records the preset in the sheet's own metadata. With a range it just resolves the role tokens."),
    archetype: z
        .enum(["tracker", "model"])
        .optional()
        .describe("tracker keeps font color out of the data language and carries state in dropdowns and conditional rules. model uses font color to encode input against formula. Taken from the preset when omitted."),
    style: z
        .object({
        role: z
            .enum(ROLE_TOKENS)
    ...
    banding: z
        ...
        .strict()
        .optional()
        .describe("Alternating row colors. Adds a banding, or updates the one already covering the range, because a second banding over the same cells is refused by the API. There is no footer color and there cannot be one: it would overwrite the last data row with SUM formulas."),
    merge: z
        .object({
    ...
    clear: z
        ...
        .strict()
        .optional()
        .describe("Strip formatting back to plain. Formats alone will not remove banding, conditional rules, or merges, which is why each is named separately. Values are never touched."),
...
export function createStyleTool(deps) {
    return {
        ...
        handler: guarded(async (raw) => {
            ...
            if (wantsTheme) {
                if (!compiled)
                    throw err.internal("A theme write reached the builder with no compiled preset.");
                requests.push({
                ...
                did.push(`applied the ${compiled.preset.name} theme, all nine slots, in ${compiled.spreadsheetTheme.primaryFontFamily}`);
                if (compiled.inheritedSlots.length) {
                    warnings.push(`The preset named no color for ${listOf(compiled.inheritedSlots)}, so the spreadsheet's existing ${compiled.inheritedSlots.length === 1 ? "value was" : "values were"} kept.`);
                }
            ...
            if (!wantsTheme || args.sheet) {
                ...
                if (args.clear) {
                    ...
                    ].filter((s) => !!s))} on ${baseRange ?? "the whole tab"}`);
                    if (baseRange)
                        touchedRanges.push(toA1Reference(sheetName, baseRange));
                }
                if (args.style) {
                    ...
                    if (!fields) {
                        throw err.invalid("style was passed with nothing in it.", "Name at least one of role, bold, background, text_color, align, wrap, number_format, or note.");
                    }
                    requests.push({
                        repeatCell: { range: gridRange(sheetId, baseRange), cell, fields },
                    });
                    did.push(`styled ${baseRange ?? "the whole tab"}${format.role ? ` with the ${format.role} role` : ""}`);
                    if (baseRange)
                        touchedRanges.push(toA1Reference(sheetName, baseRange));
                }
                if (args.borders) {
                ...
                if (args.row_heights?.length)
                    did.push(`set ${count(args.row_heights.length, "row height")}`);
                ...
                if (args.merge) {
                    ...
                    if (!mergeRange) {
                        throw err.invalid("merge needs a range.", "Pass range, or merge.range, naming the cells to merge, for example A1:F1.");
                    }
                ...
                if (args.unmerge) {
                    const unmergeRange = typeof args.unmerge === "object" ? args.unmerge.range : baseRange;
            ...
            if (requests.length === 0) {
                throw err.invalid("sheets_style was called with nothing to do.", "Pass a style, borders, column_widths, row_heights, autofit, banding, merge, unmerge, clear, freeze_rows, freeze_columns, gridlines, tab_color, or a preset.");
            }
            if (args.dry_run) {
                return ok(lines(`Dry run. Nothing was changed.`, `Would send ${count(requests.length, "request")} in one batchUpdate: ${listOf(did)}.`, warnings.length ? `\nWorth knowing:\n${warnings.map((w) => `- ${w}`).join("\n")}` : undefined), {
            ...
            const batch = await runBatchUpdate(ctx.sheets, spreadsheetId, requests);
            ...
            const check = args.check === false || gateRanges.length === 0
                ? await runErrorGate(ctx.sheets, spreadsheetId, [], {
                    skip: args.check === false
                        ? "check was false."
            ...
            return ok(lines(`${surface.spreadsheetTitle}: ${listOf(did)}, in one batchUpdate of ${count(batch.requestCount, "request")}.`, compiled && wantsTheme
                ? `Colors that name a theme slot were written as theme references, so changing Format > Theme re-skins them.`
                : undefined, check.status === "skipped" ? undefined : describeCheck(check), warnings.length ? `\nWorth knowing:\n${warnings.map((w) => `- ${w}`).join("\n")}` : undefined), structured);
        }),
    };
}
async function readSurface(ctx, spreadsheetId, sheet) {
    const response = await withRetry(() => ctx.sheets.spreadsheets.get({ spreadsheetId, fields: STYLE_MASK }));
    const data = response.data;
    const sheets = (data.sheets ?? []).flatMap((s) => {
        ...
        return [
            {
                ...
                bandedRanges: (s.bandedRanges ?? []).flatMap((b) => b.bandedRangeId === undefined || b.bandedRangeId === null
                    ? []
                    : [{ bandedRangeId: b.bandedRangeId, bounds: (b.range ?? {}) }]),
                merges: (s.merges ?? []),
    ...
    if (data.properties?.spreadsheetTheme)
        surface.currentTheme = data.properties.spreadsheetTheme;
    return surface;
}
function requireSheet(args, surface) {
    if (!args.sheet) {
        throw err.invalid("sheet is required for anything but a spreadsheet-wide theme.", "Pass the tab name. A preset with no range and no sheet writes the workbook theme, which is the only call that needs no tab.");
    }
    const wanted = args.sheet.trim().toLowerCase();
    const hit = surface.sheets.find((s) => s.title.toLowerCase() === wanted);
    if (!hit)
        throw err.sheetNotFound(args.sheet, surface.sheets.map((s) => s.title));
    return hit;
}
// ---------------------------------------------------------------------------
// Guards
// ---------------------------------------------------------------------------
function assertRestyleAllowed(args, policy, title) {
    const owner = policy?.owner;
    if (owner !== "human" && owner !== "shared")
        return;
    ...
    if (args.style && !args.range)
        restyling.push("style");
    if (restyling.length === 0)
        return;
    if (args.force !== true || !args.reason?.trim()) {
        throw new GsheetsError("needs_confirmation", `${policy?.name ?? title} is ${owner} owned, and ${listOf(restyling.map(String))} ${restyling.length === 1 ? "changes" : "change"} how it looks for everybody who uses it.`, "Applying a theme, a banding, or a tab-wide style to somebody else's spreadsheet changes tabs nobody asked about. If a person asked for this, pass force: true and reason saying who asked. If they asked for one range, pass that range instead.", { owner, restyling: restyling.map(String) });
    }
}
/**
 ...
 */
function assertMergeAllowed(range, sheet, force, sheetName) {
    const bounds = parseA1(range);
    for (const table of sheet.tables) {
        if (!overlaps(bounds, table.bounds))
            continue;
        if (force)
            return;
        throw new GsheetsError("contract_violation", `${range} overlaps the native Table${table.name ? ` "${table.name}"` : ""} on ${sheetName}.`, "A merge inside a Table breaks sorting, filtering, and every structured reference that crosses it. Put the title in a row above the Table and merge there instead.", { table: table.name ?? table.tableId, table_range: gridRangeToA1(table.bounds) });
    }
    ...
    if (frozen > 0 && startRow >= frozen && !force) {
        throw new GsheetsError("contract_violation", `${range} is inside the data region on ${sheetName}, below the ${count(frozen, "frozen header row")}.`, "Merges in a data region break sorting and filtering. Merge a title banner above the header instead, or centre the text across the cells without merging them.");
    }
}
function overlaps(a, b) {
    const axis = (aStart, aEnd, bStart, bEnd) => {
        const s1 = aStart ?? 0;
    ...
    return (axis(a.startRowIndex, a.endRowIndex, b.startRowIndex, b.endRowIndex) &&
        axis(a.startColumnIndex, a.endColumnIndex, b.startColumnIndex, b.endColumnIndex));
}
// ---------------------------------------------------------------------------
// The preset
// ---------------------------------------------------------------------------
function compilePreset(args, surface, policy) {
...
// Request builders
// ---------------------------------------------------------------------------
function gridRange(sheetId, range) {
    return a1ToGridRange(range, sheetId);
}
function buildCellFormat(style, compiled) {
    const role = typeof style["role"] === "string" ? style["role"] : undefined;
    let format = {};
    if (role) {
        if (!compiled) {
    ...
    if (format.backgroundColorStyle)
        userEnteredFormat["backgroundColorStyle"] = format.backgroundColorStyle;
    const set = (key, value) => {
        if (value !== undefined)
            userEnteredFormat[key] = value;
    };
...
function borderRequest(borders, sheetId, range) {
    const style = borders.style ?? "SOLID";
    const border = style === "NONE"
        ? { style: "NONE" }
        : {
            style,
    ...
    if (borders.edges === "all") {
        ...
        for (const edge of borders.edges)
            wanted.add(edge);
    }
    const request = { range: gridRange(sheetId, range) };
    const map = {
        top: "top",
        bottom: "bottom",
        left: "left",
 ...
 */
function bandingRequests(banding, sheetId, baseRange, sheet, compiled) {
    ...
    if (compiled && !compiled.bandingApplies) {
        warnings.push(`The ${compiled.archetype} archetype turns banding off, because banding and a font-color language fight each other for the reader's attention. This call banded anyway, which was asked for explicitly.`);
    }
 ...
 */
function clearRequests(clear, sheetId, bounds, sheet) {
    const requests = [];
    if (clear.banding) {
...
function sheetPropertyRequest(args, sheetId, compiled) {
    const gridProperties = {};
    const properties = { sheetId };
    const described = [];
    if (args.freeze_rows !== undefined) {
        gridProperties["frozenRowCount"] = args.freeze_rows;
        described.push(args.freeze_rows === 0 ? "unfroze the header" : `froze ${count(args.freeze_rows, "row")}`);
    }
    ...
    if (Object.keys(gridProperties).length)
...
function resolveTabColor(spec, compiled) {
    const key = spec.trim().toLowerCase();
    if (key === "inputs" || key === "workings" || key === "outputs") {
        const fromPreset = compiled?.tabColors[key];
        if (fromPreset)
            return fromPreset;
        throw err.invalid(`"${spec}" is a tab role, and this preset defines no color for it.`, "Pass a color, a theme slot like theme:ACCENT1, or none to clear the tab color.");
    }
 ...
 */
function manifestRequest(compiled, existing) {
...
function sheetMetadataRequest(compiled, sheetId, existing) {
    const value = JSON.stringify({ preset: compiled.preset.name, archetype: compiled.archetype });
    const hit = existing.find((e) => e.key === METADATA_KEYS.sheet && e.scope === "SHEET" && e.sheetId === sheetId);
    if (hit) {
        return {
            updateDeveloperMetadata: {
                dataFilters: [
...
async function readMetadata(ctx, spreadsheetId) {
    try {
        ...
        return toMetadataEntries((search.data.matchedDeveloperMetadata ?? []).flatMap((m) => m.developerMetadata ? [m.developerMetadata] : []));
    }
    catch {
        return [];
    }
}
function safeBounds(range) {
    try {
