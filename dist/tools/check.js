...
import { buildContract, readColumnMetadata, readSheetMetadata, readManifest, toMetadataEntries, } from "../lib/contract.js";
import { ERROR_VALUE_TYPES, LOADING_BUDGET_MS } from "../lib/errorgate.js";
import { err } from "../lib/errors.js";
import { countBySeverity, runRules, selectRules, tallyFormulaErrors, LINT_RULE_IDS, } from "../lib/lint/index.js";
...
export const checkInputSchema = {
    ...
    rules: z
        .array(z.string().min(1))
        .optional()
        .describe(`Run only these rules, by id. Everything when omitted. The v1 set is ${LINT_RULE_IDS.join(", ")}.`),
    writes: z
        .array(z.string().min(1))
        .optional()
        .describe("Ranges written in this session, as A1 with the tab name, for L14. The server already remembers its own writes; pass this only to add writes it did not make."),
};
const DESCRIPTION = [
...
export function createCheckTool(deps, toolOptions = {}) {
    ...
    return {
        ...
        handler: guarded(async (raw) => {
            ...
            if (args.range && !args.sheet && (args.sheets?.length ?? 0) !== 1) {
                throw err.invalid("range needs to know which tab it belongs to.", 'Pass sheet with the tab name alongside range, for example { "sheet": "Roster", "range": "A1:H80" }.');
            }
            const { rules: selected, unknown } = selectRules(args.rules);
            if (selected.length === 0) {
                throw err.invalid(`None of ${listOf(args.rules ?? [])} is a rule this build runs.`, `The v1 rules are ${LINT_RULE_IDS.join(", ")}. Omit rules to run all of them.`);
            }
            // --- which tabs -------------------------------------------------------
            ...
            if (targets.length === 0) {
                throw err.invalid("This spreadsheet has no visible tabs to check.", "Pass sheets with a tab name if the tab you mean is hidden.");
            }
            const notes = [];
            let checking = targets;
            if (checking.length > MAX_TABS) {
                notes.push(`This spreadsheet has ${checking.length} tabs and one call reads at most ${MAX_TABS}, so the first ${MAX_TABS} were checked. Pass sheets to name the rest.`);
                checking = checking.slice(0, MAX_TABS);
            }
            // --- read one: FORMULA values, which also give the used extent ---------
            const valueRanges = checking.map((info) => args.range && checking.length === 1 ? toA1Reference(info.title, args.range) : toA1Reference(info.title));
            const valuesResponse = await withRetry(() => ctx.sheets.spreadsheets.values.batchGet({
                spreadsheetId,
                ranges: valueRanges,
                valueRenderOption: "FORMULA",
                dateTimeRenderOption: "FORMATTED_STRING",
            }));
            const formulaGrids = new Map();
            const valueList = valuesResponse.data.valueRanges ?? [];
            checking.forEach((info, index) => {
                const values = (valueList[index]?.values ?? []);
            ...
            for (const info of checking) {
                ...
                if (cells > budget) {
                    ...
                    if (affordableRows < rows)
                        trimmed += 1;
                    gridRanges.push(toA1Reference(info.title, `A1:${columnIndexToLetter(columns - 1)}${Math.max(1, affordableRows)}`));
                    budget = 0;
                }
                else {
                    budget -= cells;
            ...
            if (trimmed > 0) {
                notes.push(`The grid read was capped at ${MAX_GRID_CELLS.toLocaleString("en-US")} cells, so ${count(trimmed, "tab")} was read only as far as that. Check the rest with range.`);
            }
            ...
            for (const info of checking) {
                ...
                if (policy)
                    lintContext.policy = policy;
                const contract = contracts.get(info.title);
                if (contract)
                    lintContext.contract = contract;
            ...
            if (unknown.length > 0) {
                notes.push(`${listOf(unknown)} ${unknown.length === 1 ? "is not a rule" : "are not rules"} this build runs, so ${unknown.length === 1 ? "it was" : "they were"} skipped. The v1 rules are ${LINT_RULE_IDS.join(", ")}.`);
            }
            const errorSummary = {};
            ...
            if (status === "pending") {
                notes.push(`${count(loading, "cell")} were still calculating after ${Math.round(loadingBudgetMs / 1000)} seconds. That is Sheets working, not a failure. Run this again in a moment.`);
            }
            const structured = {
            ...
            return ok(lines(headline(status, findings.length, counts, totalFormulas, totalErrors, checking.length), findings.length ? "" : undefined, ...findings.map(findingLine), notes.length ? `\n${notes.map((note) => `- ${note}`).join("\n")}` : undefined, "\nNo dropdown paints as a pill in a render, so this is the authority on validation state. A clean check means the formulas evaluate. It does not mean they point where you think they do, so spot-check a number by hand."), structured, { maxResultSizeChars: 120_000 });
        }),
    };
}
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function headline(status, findingCount, counts, formulas, errors, tabs) {
...
function describeCounts(counts) {
    const parts = [];
    if (counts.error)
        parts.push(count(counts.error, "error"));
    if (counts.warning)
        parts.push(count(counts.warning, "warning"));
    if (counts.info)
        parts.push(count(counts.info, "info item"));
    return parts.length ? `${listOf(parts)}.` : "";
}
function findingLine(finding) {
    return `  [${finding.severity}] ${finding.rule} ${finding.location}: ${finding.message}\n      Fix: ${finding.fix}`;
}
function countLoading(sheets) {
    let loading = 0;
    for (const sheet of sheets) {
        for (const block of sheet.data ?? []) {
            for (const row of block?.rowData ?? []) {
                for (const cell of row?.values ?? []) {
                    if (cell?.effectiveValue?.errorValue?.type === "LOADING")
                        loading += 1;
                }
            }
        }
...
function parseWrites(ranges) {
    if (!ranges?.length)
        return [];
    const now = Date.now();
    return ranges.map((range) => {
        const match = /^'?([^'!]+)'?!/.exec(range.trim());
        const entry = { range: range.trim(), at: now };
        if (match)
            entry.sheet = match[1];
        return entry;
    });
 ...
 */
async function loadContracts(ctx, spreadsheetId, targets, formulaGrids) {
    const out = new Map();
    let entries = [];
    try {
        const response = await withRetry(() => ctx.sheets.spreadsheets.developerMetadata.search({
            spreadsheetId,
