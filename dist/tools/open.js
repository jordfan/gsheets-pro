...
import { buildContract, inferConventions, markUiOwned, readColumnMetadata, readManifest, readSheetMetadata, toMetadataEntries, } from "../lib/contract.js";
...
const VALIDATION_MASK = "sheets.properties(sheetId,title),sheets.data.rowData.values(dataValidation(condition(type,values(userEnteredValue)),strict,inputMessage))";
export const openInputSchema = {
    spreadsheet_id: z
        .string()
        .optional()
        .describe(`${SPREADSHEET_ID_DESCRIPTION} Omit only when creating a new spreadsheet.`),
    sheet: z
        .string()
        .optional()
    ...
    include_validation: z
        .boolean()
        .optional()
        .describe("Read the data validation rules on the first data row of each tab, so dropdowns set in the UI are reported rather than overwritten later. Default true."),
    sample_rows: z
        .number()
        .int()
        .min(0)
...
export function createOpenTool(deps) {
    return {
        name: "sheets_open",
        config: {
            title: "Open a spreadsheet",
            description: "Call this first, before reading or writing anything. Returns the map of a spreadsheet: tabs and their sizes, native Tables, named ranges, protected ranges, the contract that says which columns are ours to write, dropdowns that belong to the Sheets UI, and the conventions the sheet already follows. Can also create a new spreadsheet.",
        ...
        handler: guarded(async (raw) => {
            ...
            if (args.create) {
                ...
                const res = await withRetry(() => ctx.sheets.spreadsheets.create({
                    requestBody: {
            ...
            if (!spreadsheetId) {
                throw err.invalid("spreadsheet_id is required.", "Pass the long id from the middle of the sheet's URL, or pass create to make a new spreadsheet.");
            }
            const meta = await withRetry(() => ctx.sheets.spreadsheets.get({ spreadsheetId, fields: SPREADSHEET_MASK }));
            ...
            ctx.cache.prime(spreadsheetId, allSheets.flatMap((s) => {
                const p = s.properties;
                if (!p || p.sheetId === undefined || p.sheetId === null || !p.title)
                    return [];
            ...
            const titles = allSheets.map((s) => s.properties?.title).filter((t) => !!t);
            ...
            try {
                ...
                metadataNote = `Developer metadata could not be read (${error.message}). Treating this spreadsheet as having none.`;
            }
            const manifest = readManifest(metadataEntries);
            const sampleRows = args.sample_rows ?? SAMPLE_ROWS;
            const sampleByTab = new Map();
            if (sampleRows > 0 && detailedTitles.length > 0) {
                const ranges = detailedTitles.map((t) => `${quoteSheetName(t)}!A1:Z${sampleRows}`);
                const values = await withRetry(() => ctx.sheets.spreadsheets.values.batchGet({
                    spreadsheetId,
                    ranges,
            ...
            if (wantValidation) {
                try {
                    ...
                    const grid = await withRetry(() => ctx.sheets.spreadsheets.get({
                        spreadsheetId,
                        ranges,
                    ...
                    for (const sheet of grid.data.sheets ?? []) {
                        ...
                        row.forEach((cell, columnIndex) => {
                            if (cell?.dataValidation)
                                found.push({ column: columnIndex, rule: cell.dataValidation });
                        });
                        if (found.length)
                            validationByTab.set(sheetId, found);
                    }
                }
                catch (error) {
                    metadataNote = lines(metadataNote, `Data validation could not be read (${error.message}).`);
                }
            }
            const warnings = [];
            if (metadataNote)
                warnings.push(metadataNote);
            const tabs = [];
            for (const sheet of detailed) {
                ...
                const validation = markUiOwned((validationByTab.get(sheetId) ?? []).map(({ column, rule }) => {
                    const r = rule;
                    ...
                    if (r.condition?.type)
                        summary.conditionType = r.condition.type;
                    return summary;
                }), pluginColumns, columnLetterToIndex);
                const report = {
                    name: title,
                    sheetId,
                    ...
                    tables: (sheet.tables ?? []).map((t) => {
                        const entry = {
                            ...
                            columns: (t.columnProperties ?? []).map((c) => {
                                const col = { name: c.columnName ?? "" };
                                if (c.columnType)
                                    col.type = c.columnType;
                                return col;
                            }),
                        };
                        if (t.name)
                ...
                for (const d of contract.drift)
                    warnings.push(`${title}: ${d}`);
                const uiOwned = validation.filter((v) => v.uiOwned).map((v) => v.column ?? v.range);
                if (uiOwned.length) {
                    warnings.push(`${title}: dropdowns on ${listOf(uiOwned)} were set outside this plugin. Their chip colours cannot be read through the API and a rewrite would lose them, so they are left alone unless you pass force.`);
                }
...
function describeTheme(theme) {
    ...
    for (const entry of theme.themeColors ?? []) {
        ...
        const hex = colorStyleToText(entry.color);
        if (slot && hex)
            colors[slot] = hex;
    }
    return {
...
function proseFor(structured, tabs, titles, created, warnings) {
    const spreadsheet = structured["spreadsheet"];
    const registry = structured["registry"];
    const namedRanges = structured["namedRanges"];
    const head = created
    ...
    const body = tabs.map((tab) => {
        const bits = [];
        bits.push(`${tab.rows ?? "?"} rows by ${tab.columns ?? "?"} columns`);
        if (tab.frozenRows)
            bits.push(`${count(tab.frozenRows, "frozen row")}`);
        if (tab.isTable)
            bits.push(`${count(tab.tables.length, "native Table")}`);
        if (tab.banded)
            bits.push("banded");
        if (tab.filtered)
            bits.push("has a filter");
        if (tab.merges)
            bits.push(`${count(tab.merges, "merged range")}`);
        if (tab.protections.length)
            bits.push(`${count(tab.protections.length, "protected range")}`);
        ...
        const contractLine = tab.contract && tab.contract.source !== "none"
            ? `  Contract: ${tab.contract.summary}`
            : undefined;
        const validationLine = tab.validation.length
            ? `  Dropdowns on ${listOf(tab.validation.map((v) => v.column ?? v.range))}${tab.validation.some((v) => v.uiOwned) ? ", set outside this plugin" : ""}.`
            : undefined;
    ...
    if (uncontracted.length) {
        const which = uncontracted.length === tabs.length
            ? "No contract."
