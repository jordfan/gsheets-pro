 ...
 */
import { z } from "zod";
import { columnIndexToLetter, columnLetterToIndex, gridRangeToA1, parseA1, quoteSheetName, toA1Reference, } from "../lib/a1.js";
import { withRetry } from "../lib/batch.js";
import { buildContract, readColumnMetadata, readManifest, readSheetMetadata, toMetadataEntries, } from "../lib/contract.js";
import { err, GsheetsError } from "../lib/errors.js";
import { describeCheck, runErrorGate, withinGateCap, } from "../lib/errorgate.js";
...
import { checkTexts, colleagueSafeApplies, describeFindings, } from "../lib/safetext.js";
...
export const writeInputSchema = {
    ...
    mode: z
        ...
        .optional()
        .describe("range (default) writes a block at an A1 range. append adds rows at the end of the data. upsert matches on a key column and updates the rows it finds, appending the rest. fill writes one formula and autofills it down, adjusting its references. log appends a row and stamps it with today's date."),
    range: z
        .string()
        .optional()
    ...
    rows: z
        ...
        .optional()
        .describe("The batch form of upsert. Every key is resolved from one read, every change goes in one write, and the guards run once for the whole batch. Prefer this over one call per row."),
    key_column: z
        .string()
        .optional()
    ...
    raw: z
        .boolean()
        .optional()
        .describe("Write every value literally, so a leading = stays text and a date-shaped string stays a string. Columns a Table or the contract types as TEXT are written this way regardless."),
    force: z
        .boolean()
        .optional()
        .describe("Override the formula guard, the writable-column contract, and the colleague-safe text check. Read what the refusal named before passing this."),
    expect_contract: z
        .object({
        ...
        .optional()
        .describe("What you believe about this sheet. The write is refused if the sheet disagrees, which is how a plan made three calls ago stops being acted on after somebody edited the tab."),
    dry_run: z
        .boolean()
        .optional()
    ...
    check: z
        .boolean()
        .optional()
        .describe("Read the written range back and report whether it still evaluates. Default true. Set false for the interior writes of a build and let the last one answer."),
};
    ...
    return {
        ...
        handler: guarded(async (raw) => {
            ...
            if (refusedColumns.length && !args.force) {
                throw new GsheetsError("contract_violation", `This write touches ${count(refusedColumns.length, "column")} that ${refusedColumns.length === 1 ? "is" : "are"} not ours on ${sheetName}. ${refusedColumns.join(" ")}`, "Write only the columns the contract marks as ours. If this genuinely is our column and the registry is out of date, fix the registry rather than passing force.", { columns: refusedColumns });
            }
            if (refusedColumns.length) {
                warnings.push(`force was passed, so ${count(refusedColumns.length, "column")} outside the contract ${refusedColumns.length === 1 ? "was" : "were"} written: ${refusedColumns.join(" ")}`);
            }
            ...
            if (formulaHits.length && !args.force) {
                ...
                throw new GsheetsError("formula_guard", `${count(formulaHits.length, "cell")} in the target range currently ${formulaHits.length === 1 ? "holds a formula" : "hold formulas"}: ${named.join("; ")}${formulaHits.length > NAME_LIMIT ? `; and ${formulaHits.length - NAME_LIMIT} more` : ""}.`, "Replacing a formula with the value it happened to produce is silent and permanent. Read those cells, decide whether you meant to, and pass force if you did. To change what a formula computes, write the new formula rather than its result.", { cells: formulaHits.slice(0, NAME_LIMIT * 4) });
            }
            if (formulaHits.length) {
                warnings.push(`force was passed, so ${count(formulaHits.length, "formula")} ${formulaHits.length === 1 ? "was" : "were"} replaced: ${formulaHits.slice(0, NAME_LIMIT).map((h) => h.cell).join(", ")}.`);
            }
            ...
            if (refusals.length && !args.force) {
                throw new GsheetsError("contract_violation", `${count(refusals.length, "value")} would not read as a colleague's on ${sheetName}, which is ${policy?.owner === "human" ? "somebody else's spreadsheet" : "shared"}. ${describeFindings(refusals)}`, "Somebody opens this sheet with no idea an agent touched it. Rewrite the text as a sentence a colleague would have typed. If this vocabulary is legitimate here, add it to the sheet's allowlist in .claude/gsheets-pro.json rather than passing force.", { findings: refusals.slice(0, 20) });
            }
            ...
            const check = args.check === false
                ? await runErrorGate(ctx.sheets, spreadsheetId, [], {
                ...
                : await runErrorGate(ctx.sheets, spreadsheetId, gateRanges);
            const structured = {
            ...
            return ok(lines(`${describePlan(plan, sheetName, written)}`, plan.placement, describeCheck(check), warnings.length ? `\nWorth knowing:\n${warnings.map((w) => `- ${w}`).join("\n")}` : undefined), structured);
        }),
    };
}
const TABLE_MASK = "sheets.properties(sheetId,title),sheets.tables(tableId,name,range,columnProperties(columnIndex,columnName,columnType))";
async function readSurface(ctx, spreadsheetId, info, args, mode) {
    const warnings = [];
    ...
    let tables = [];
    try {
        const response = await withRetry(() => ctx.sheets.spreadsheets.get({ spreadsheetId, fields: TABLE_MASK }));
        for (const sheet of response.data.sheets ?? []) {
            if (sheet.properties?.sheetId !== info.sheetId)
                continue;
            tables = (sheet.tables ?? []).map((t) => ({
                tableId: t.tableId ?? "",
                ...(t.name ? { name: t.name } : {}),
                bounds: (t.range ?? {}),
                columnTypes: new Map((t.columnProperties ?? []).flatMap((c) => c.columnIndex !== undefined && c.columnIndex !== null && c.columnType
                    ? [[c.columnIndex, c.columnType]]
                    : [])),
            }));
        }
    }
    catch (error) {
        warnings.push(`The tab's native Tables could not be read (${error.message}), so this write treated the tab as a plain range.`);
    }
    ...
    const response = await withRetry(() => ctx.sheets.spreadsheets.values.batchGet({
        spreadsheetId,
        ranges: [scan],
        valueRenderOption: "FORMULA",
        majorDimension: "ROWS",
    }));
    const grid = (response.data.valueRanges?.[0]?.values ?? []);
    const headerRow = resolveHeaderRow(args.header_row, info.frozenRowCount, grid);
    const headers = headerRow !== undefined
        ? normalizeHeaders((grid[headerRow - 1] ?? []).map((c) => (c === null || c === undefined ? "" : String(c))))
        : [];
    const firstDataRow = headerRow !== undefined ? headerRow + 1 : 1;
    const blocks = findBlocks(grid, firstDataRow);
    let metadataEntries = [];
    try {
        const search = await withRetry(() => ctx.sheets.spreadsheets.developerMetadata.search({
            spreadsheetId,
            requestBody: { dataFilters: [{ developerMetadataLookup: {} }] },
        }));
        metadataEntries = toMetadataEntries((search.data.matchedDeveloperMetadata ?? []).flatMap((m) => m.developerMetadata ? [m.developerMetadata] : []));
    }
    catch {
    ...
    if (blocks.multipleBlocks && (mode === "append" || mode === "log")) {
        warnings.push(`A blank row splits this tab into more than one block of data. The append targets the end of the first block, row ${blocks.lastDataRow}, not the bottom of the tab.`);
    }
    const surface = {
        headers,
        firstDataRow,
    ...
    if (headerRow !== undefined)
        surface.headerRow = headerRow;
    if (policy)
        surface.policy = policy;
    return surface;
}
/** One based header row: what was asked for, then frozen rows, then the first text row. */
export function resolveHeaderRow(headerRow, frozenRowCount, grid) {
    if (headerRow !== undefined)
        return headerRow;
    if (frozenRowCount && frozenRowCount > 0)
    ...
    for (let i = 0; i < Math.min(grid.length, 10); i += 1) {
        ...
        if (filled.length < 2)
            continue;
        if (filled.every((c) => typeof c !== "number" && !Number.isFinite(Number(String(c)))))
            return i + 1;
    }
 ...
 */
export function findBlocks(grid, firstDataRow) {
    const filled = (i) => (grid[i] ?? []).some((c) => String(c ?? "").trim() !== "");
...
// expect_contract
// ---------------------------------------------------------------------------
function assertExpectedContract(expected, contract, headers, sheetName) {
    if (!expected)
        return;
    const mismatches = [];
    ...
    if (expected.positional_rows !== undefined && expected.positional_rows !== contract.positionalRows) {
        mismatches.push(contract.positionalRows
    ...
    if (expected.headers) {
        ...
        const differs = wanted.length !== actual.length || wanted.some((h, i) => h !== actual[i]);
        if (differs) {
    ...
    if (mismatches.length) {
        throw new GsheetsError("contract_violation", `${sheetName} is not what this write expected: ${listOf(mismatches)}.`, "Somebody changed the tab since the plan was made. Call sheets_open again, look at what changed, and decide whether the write still means what it meant.", { mismatches });
    }
}
...
function valueInputFor(columnIndex, args, surface) {
    if (args.raw === true)
        return "RAW";
    for (const table of surface.tables) {
        if (!containsColumn(table.bounds, columnIndex))
            continue;
        const start = table.bounds.startColumnIndex ?? 0;
        if (table.columnTypes.get(columnIndex - start) === "TEXT")
            return "RAW";
        if (table.columnTypes.get(columnIndex) === "TEXT")
            return "RAW";
    }
    const column = surface.contract.columns[columnIndex];
    if (column?.type && column.type.toUpperCase() === "TEXT")
        return "RAW";
    return "USER_ENTERED";
}
function containsColumn(bounds, columnIndex) {
...
function planRange(input) {
    const { args, surface, info } = input;
    const rangeA1 = args.range?.trim();
    if (!rangeA1) {
        throw err.invalid("range is required for mode range.", "Pass an A1 range like B2:E20. To add rows at the end of the data instead, use mode append.");
    }
...
function planAppend(input, isLog) {
    const { args, surface, info } = input;
    ...
    if (surface.multipleBlocks && args.at_row === undefined) {
        throw new GsheetsError("invalid_argument", `${info.title} holds more than one block of data, so there is no single "end" to append to.`, `The first block ends at row ${surface.lastDataRow}. Writing there would land on the blank row that separates the blocks, and appending past everything would land inside the second block. Pass at_row to say exactly where these rows go, or use sheets_structure to insert rows first.`, { last_data_row: surface.lastDataRow });
    }
...
async function planUpsert(input) {
    const { args, surface, info, ctx, spreadsheetId } = input;
    const entries = upsertEntries(args, surface.headers);
    if (entries.length === 0) {
        throw err.invalid("Nothing to upsert: rows and records were both empty.", "The batch form is rows: [{ key, set }], which resolves every key from one read and writes once.");
    }
    const keyName = args.key_column ?? surface.contract.keyColumn;
    if (!keyName) {
        throw err.invalid("key_column is required for an upsert and the contract does not name one.", surface.headers.length
    ...
    const keyResponse = await withRetry(() => ctx.sheets.spreadsheets.values.batchGet({
        spreadsheetId,
        ranges: [`${quoteSheetName(info.title)}!${keyLetter}1:${keyLetter}${SCAN_ROWS}`],
        valueRenderOption: "FORMATTED_VALUE",
        majorDimension: "COLUMNS",
    }));
    const keyColumn = ((keyResponse.data.valueRanges?.[0]?.values ?? [[]])[0] ?? []);
    ...
    for (let i = surface.firstDataRow - 1; i < keyColumn.length && i < surface.lastDataRow; i += 1) {
        ...
        if (rowByKey.has(lower))
            duplicates.add(value);
        else
            rowByKey.set(lower, i + 1);
    }
    if (duplicates.size) {
        surface.warnings.push(`The key column ${keyLetter} repeats ${listOf([...duplicates].slice(0, 5).map((d) => `"${d}"`))}. An upsert updated the first row for each; the others were left alone.`);
    }
    const updates = [];
    const inserts = [];
    ...
    if (inserts.length) {
        ...
        if (table) {
            ...
            throw new GsheetsError("invalid_argument", `${count(inserts.length, "row")} did not match an existing key and would have to be appended, but ${info.title} holds more than one block of data.`, `The first block ends at row ${surface.lastDataRow}. Pass at_row to say where new rows go, or upsert only the keys that already exist.`, { unmatched: inserts.length, last_data_row: surface.lastDataRow });
        }
        else {
            const startRow = (args.at_row ?? surface.lastDataRow + 1) - 1;
    ...
    plan.placement = lines(plan.matched
        ? `Matched ${count(plan.matched, "row")} on ${keyLetter} (${key.header ?? keyLetter}) and updated only the named fields.`
        : `No existing row matched on ${keyLetter}.`, plan.inserted
        ? `${count(plan.inserted, "row")} did not match and ${plan.inserted === 1 ? "was" : "were"} appended.`
        : undefined);
    return plan;
}
function planFill(input) {
    const { args, surface, info } = input;
    const rangeA1 = args.range?.trim();
    if (!rangeA1) {
        throw err.invalid("range is required for mode fill.", "Pass the whole region the formula should cover, including the row it starts in, for example E2:E60.");
    }
    const bounds = parseA1(rangeA1);
    if (bounds.startRowIndex === undefined || bounds.endRowIndex === undefined) {
        throw err.invalid(`fill needs a range bounded on both ends, and "${rangeA1}" is open.`, "Say where the fill stops, for example E2:E60. A whole-column fill would write to the bottom of the sheet.");
    }
    const rows = bounds.endRowIndex - bounds.startRowIndex;
    if (rows < 2) {
        throw err.invalid(`"${rangeA1}" is one row, so there is nothing to fill down into.`, "The range covers the seed row and every row the formula should reach.");
    }
    const ranges = [];
    if (args.formula) {
        const width = (bounds.endColumnIndex ?? 1) - (bounds.startColumnIndex ?? 0);
        if (width !== 1) {
            throw err.invalid("formula fills one column at a time.", `"${rangeA1}" is ${width} columns wide. Call fill once per column, or leave formula out and let the first row of the range be the seed as it already stands.`);
        }
        const seed = {
...
// Value shaping
// ---------------------------------------------------------------------------
function gridFromArgs(args, headers, startColumn) {
    if (args.values?.length)
        return args.values.map((r) => [...r]);
    if (args.records?.length) {
        return args.records.map((record) => recordToRow(record, headers).slice(startColumn));
    }
    return [];
}
function rowsFromArgs(args, headers, isLog, timestampColumn) {
    let rows;
    if (args.values?.length)
        rows = args.values.map((r) => [...r]);
    else if (args.records?.length)
        rows = args.records.map((record) => recordToRow(record, headers));
    else
        return [];
    if (!isLog)
        return rows;
...
function recordToRow(record, headers) {
    if (headers.length === 0) {
        throw err.invalid("records need a header row to know which column each field belongs in.", "Pass header_row to say which row holds the headers, or pass values as rows of cells instead.");
    }
    const row = new Array(headers.length).fill("");
    for (const [field, value] of Object.entries(record)) {
        const column = resolveColumn(field, headers);
        while (row.length <= column.index)
            row.push("");
        row[column.index] = value;
    }
    return row;
}
function upsertEntries(args, headers) {
    if (args.rows?.length)
        return args.rows.map((r) => ({ key: r.key, set: r.set }));
    if (args.records?.length) {
        const keyName = args.key_column;
        if (!keyName) {
            throw err.invalid("records with mode upsert need key_column, so each record can say which row it is.", "Or use the batch form, rows: [{ key, set }], which states the key explicitly.");
        }
        ...
        return args.records.map((record) => {
            ...
            if (key === undefined || key === null || String(key).trim() === "") {
                throw err.invalid(`A record has no value in the key column "${header}".`, "Every record in an upsert needs its key, or there is no way to know which row it means.");
            }
...
export function resolveColumn(nameOrLetter, headers) {
    const raw = String(nameOrLetter ?? "").trim();
    if (!raw)
        throw err.invalid("A column was named with an empty string.");
    ...
    if (/^[A-Za-z]{1,3}$/.test(raw)) {
        ...
        if (headers[index])
            found.header = headers[index];
        return found;
    }
    throw err.invalid(`No column named "${nameOrLetter}".`, headers.length
...
export function coalesce(indexes) {
 ...
 */
function splitByValueInput(bounds, values, args, surface, sheetName) {
    const startColumn = bounds.startColumnIndex ?? 0;
    const startRow = bounds.startRowIndex ?? 0;
    const width = Math.max(...values.map((r) => r.length), 0);
...
function tableForData(surface) {
    if (surface.tables.length === 0)
        return undefined;
    if (surface.tables.length === 1)
        return surface.tables[0];
...
function columnRefusal(index, headers, contract, policy) {
 ...
 */
async function findFormulaCollisions(ctx, spreadsheetId, sheetName, plan) {
    ...
    (response.data.valueRanges ?? []).forEach((valueRange, i) => {
        ...
        grid.forEach((row, r) => {
            row.forEach((existing, c) => {
                ...
                if (planned && typeof replacement === "string" && replacement === existing)
                    return;
                if (planned && replacement === undefined)
                    return;
                hits.push({
...
function runSafeTextCheck(plan, sheetName, policy) {
    if (!colleagueSafeApplies(policy))
        return [];
...
async function execute(ctx, spreadsheetId, sheetName, plan, args) {
    const result = { ranges: [], updatedCells: 0, updatedRows: 0, calls: [] };
    ...
    for (const [option, ranges] of byOption) {
        const response = await withRetry(() => ctx.sheets.spreadsheets.values.batchUpdate({
            spreadsheetId,
            requestBody: {
                valueInputOption: option,
    ...
    if (plan.append) {
        const response = await withRetry(() => ctx.sheets.spreadsheets.values.append({
            spreadsheetId,
            range: plan.append.tableRange,
            valueInputOption: plan.append.valueInputOption,
            insertDataOption: "INSERT_ROWS",
            requestBody: { values: plan.append.values },
        }));
        ...
        if (landed && plan.append.rawColumns.length && plan.append.valueInputOption !== "RAW") {
            ...
            await withRetry(() => ctx.sheets.spreadsheets.values.batchUpdate({
                spreadsheetId,
    ...
    if (plan.fill) {
        await withRetry(() => ctx.sheets.spreadsheets.batchUpdate({
            spreadsheetId,
            requestBody: {
                requests: [
                    {
...
function describePlan(plan, sheetName, written) {
...
function dryRunResponse(spreadsheetId, sheetName, plan, warnings, headerRow) {
    ...
    return ok(lines(`Dry run. Nothing was written.`, plan.placement, preview.length
        ? `Would write:\n${preview.map((p) => `- ${p.range}, ${count(p.rows, "row")}, ${p.value_input}`).join("\n")}`
        : undefined, plan.append
