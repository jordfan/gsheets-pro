...
import { DEFAULT_DPI, exportUrl, fetchExportPdf, pdfToPng, renderFileName, serializeRender, } from "../lib/render.js";
...
export function createRenderTool(deps, toolOptions = {}) {
    return {
        name: "sheets_render",
        config: {
        ...
        handler: guarded(async (raw) => {
            ...
            const options = { gid: info.sheetId };
            if (args.gridlines !== undefined)
                options.gridlines = args.gridlines;
            if (args.portrait !== undefined)
                options.portrait = args.portrait;
            if (args.range)
                Object.assign(options, rangeParams(args.range));
            const dir = renderDir();
            ...
            return serializeRender(async () => {
                ...
                try {
                    ...
                    bytes = (await fetchExportPdf(exportUrl(spreadsheetId, options), token, toolOptions.fetchImpl ?? fetch)).bytes;
                }
                catch (error) {
                    ...
                    bytes = fallback;
                    wholeWorkbook = true;
                    warnings.push(`The export endpoint did not answer (${why}), so this is Drive's own export: the whole workbook, every tab, with gridlines, and no way to honour ${args.range ? "the range or " : ""}the page settings. Page 1 is whichever tab comes first, not necessarily ${info.title}.`);
                }
                ...
                if (conversion.scaledTo) {
                    warnings.push(`The page was wider than ${conversion.scaledTo} pixels, so it was scaled down to fit. Small text may be soft. Render a range instead of the whole tab to read it at full size.`);
                }
                if (conversion.paths.length > 1) {
                    warnings.push(`This render is ${count(conversion.paths.length, "page")}. The frozen header repeats on each page of grid.`);
                    ...
                    // that is not there.
                    warnings.push(`If this tab has cell notes, the last page is not more grid. Notes render as endnotes: the note bodies are printed there as a numbered list, and the bracketed markers in the grid, "Instructor [1]", are references to that list rather than text anybody typed into the cell. Keep that page. It is the sheet's own documentation, and dropping it is how a well-documented sheet comes back looking undocumented.`);
                }
                const hosted = renderMode() === "hosted";
                const outputs = conversion.paths.map((file, index) => {
                    const name = path.basename(file);
                    const entry = { page: index + 1, file: name };
                    if (hosted) {
                ...
                if (hosted && !renderPublicUrl()) {
                    warnings.push("This server does not know its own public address, so the URLs are paths rather than absolute. Set GSHEETS_PRO_PUBLIC_URL on the host to fix it.");
                }
                const structured = {
                ...
                const where = hosted
                    ? outputs.map((o) => `  pages[${o.page - 1}].url: ${o.url}`).join("\n")
                    : outputs.map((o) => `  pages[${o.page - 1}].path: ${o.path}`).join("\n");
                return ok(lines(`Rendered ${info.title}${args.range ? `!${args.range}` : ""} at ${args.dpi ?? DEFAULT_DPI} dpi, ${count(outputs.length, "page")}.`, where, hosted
                    ? `\nThe links are valid for ${Math.round(RENDER_URL_TTL_MS / 60000)} minutes. Fetch them now rather than later.`
                    : "\nRead the file at that path to look at it.", warnings.length ? `\nWorth knowing:\n${warnings.map((w) => `- ${w}`).join("\n")}` : undefined, `\n${RENDER_CAVEAT}`), structured);
            });
        }),
    };
}
...
export function rangeParams(range) {
    ...
    try {
        ...
        throw err.badRange(range, error.message);
    }
    const out = {};
    if (bounds.startRowIndex !== undefined)
        out.r1 = bounds.startRowIndex;
    if (bounds.startColumnIndex !== undefined)
        out.c1 = bounds.startColumnIndex;
    if (bounds.endRowIndex !== undefined)
        out.r2 = bounds.endRowIndex;
    if (bounds.endColumnIndex !== undefined)
        out.c2 = bounds.endColumnIndex;
    return out;
}
...
async function accessToken(ctx) {
    ...
    if (!token) {
        throw err.authMissing("The credential did not return an access token.", "Run `gsheets-pro doctor` to see which credential is in use, then `gsheets-pro auth` to sign in again.");
    }
    return token;
}
/** Drive's documented export. The whole workbook, and nothing to tune. */
async function driveExport(ctx, spreadsheetId) {
    const response = await withRetry(() => ctx.drive.files.export({ fileId: spreadsheetId, mimeType: "application/pdf" }, { responseType: "arraybuffer" }));
