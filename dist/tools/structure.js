 ...
 */
import { z } from "zod";
import { columnIndexToLetter, columnLetterToIndex, gridRangeToA1, parseA1, parseColumnSpan, parseRowSpan, quoteSheetName, } from "../lib/a1.js";
...
import { assertWritable, describeSheet } from "../lib/registry.js";
...
export const ROW_ORDER_ACTIONS = [
    "insert_rows",
    "delete_rows",
    "move_rows",
    "sort",
...
export const structureInputSchema = {
    ...
    sheet: z
        .string()
        .optional()
        .describe("The tab to act on. Required by every action except add_tab, which creates one."),
    title: z
        .string()
        .optional()
    ...
    range: z
        .string()
        .optional()
        .describe("A1 range within the tab. sort, dedupe, trim, find_replace, protect, unprotect. Sort and dedupe default to everything below the frozen header."),
    rows: z
        .string()
        .optional()
        .describe('A band of rows, written as "5:9" or "12". insert_rows inserts that many rows starting there; delete_rows, move_rows, group and ungroup act on them.'),
    columns: z
        .string()
        .optional()
        .describe('A band of columns, written as "B:D" or "C". insert_columns inserts that many starting there; delete_columns, move_columns, group and ungroup act on them.'),
    to: z
        .string()
        .optional()
        .describe('move_rows: the row number to put them before, for example "12". move_columns: the column letter to put them before, for example "B".'),
    sort_by: z
        .array(z.object({
    ...
    include_formulas: z
        .boolean()
        .optional()
        .describe("find_replace: also rewrite text inside formulas. Default false, because rewriting a colleague's formula is how a sheet breaks quietly."),
    key_columns: z
        .array(z.string().min(1))
        .optional()
        .describe("dedupe: the columns that decide whether two rows are duplicates, by letter or header name. Defaults to every column in the range."),
    description: z
        .string()
        .optional()
        .describe("protect: what the protection is for, in words a colleague will read when Sheets warns them. Required."),
    warning_only: z
        .boolean()
        .optional()
        .describe("protect: true (the default) warns a person before they edit and lets them continue. false locks everyone but this account out and needs confirm."),
    ...
    confirm: z
        .string()
        .optional()
        .describe("Required by delete_tab, delete_rows, delete_columns, dedupe and find_replace, and by protect when warning_only is false. Set it to the tab's own name."),
...
export function createStructureTool(deps) {
    return {
        ...
        handler: guarded(async (raw) => {
            ...
            const info = action === "add_tab" ? undefined : await ctx.cache.resolve(spreadsheetId, requireSheet(args));
            ...
            if (action === "copy_tab_to")
                return copyTabTo(ctx, args, info);
            const built = await buildRequests(ctx, args, info, policy);
            const result = await runBatchUpdate(ctx.sheets, spreadsheetId, built.requests);
            ...
            if (GATED_ACTIONS.includes(action) && built.gateRanges.length > 0) {
                check = await runErrorGate(ctx.sheets, spreadsheetId, built.gateRanges);
            }
            const warnings = [...built.warnings];
            if (SHIFTING_ACTIONS.includes(action))
                warnings.push(shiftWarning(action));
            if (policy && policy.owner !== "agent") {
                warnings.push(`${describeSheet(policy)} lists this spreadsheet as ${policy.owner}, so a colleague will see this change.`);
            }
            const structured = {
            ...
            const text = lines(built.summary, check ? describeCheck(check) : undefined, warnings.length ? `\nWorth knowing:\n${warnings.map((w) => `- ${w}`).join("\n")}` : undefined);
...
// Guards
// ---------------------------------------------------------------------------
function requireSheet(args) {
    if (!args.sheet?.trim()) {
        throw err.invalid(`${args.action} needs a sheet.`, "Pass the tab name as it reads on the tab strip. Only add_tab works without one.");
    }
    return args.sheet;
}
function guardPositionalRows(action, policy, sheet) {
    if (!policy?.positionalRows)
        return;
    if (!ROW_ORDER_ACTIONS.includes(action))
        return;
    throw new GsheetsError("contract_violation", `${action} is refused on ${sheet ? `"${sheet}"` : "this sheet"}: the registry marks its rows as positional.`, `${describeSheet(policy)} says rows here are referred to by position somewhere else, so sorting, inserting, deleting or moving them would silently invalidate that. Change the values in place instead, or take the row numbering out of the registry entry if it is no longer true.`, { policy: policy.path, action });
}
function guardConfirmation(action, args, sheet) {
    const hardProtect = action === "protect" && args.warning_only === false;
    if (!DESTRUCTIVE_ACTIONS.includes(action) && !hardProtect)
        return;
    ...
    const what = hardProtect
        ...
        : `${action.replace(/_/g, " ")}`;
    throw new GsheetsError("needs_confirmation", given
        ? `confirm was "${given}" and this action needs "${expected}".`
        : `${what} needs confirm.`, `Pass confirm: "${expected}", the tab's own name, to say this is deliberate. Nothing has been changed.`, { action, expected });
}
async function buildRequests(ctx, args, info, policy) {
    const warnings = [];
    ...
    switch (args.action) {
        ...
        case "rename_tab": {
            const title = requireString(args.title, "rename_tab needs a title.", "Pass the new name for the tab.");
            return {
                requests: [
        ...
        case "duplicate_tab": {
            const request = { sourceSheetId: info.sheetId };
            if (args.title)
                request["newSheetName"] = args.title;
            if (args.index !== undefined)
                request["insertSheetIndex"] = args.index;
            return {
        ...
        case "delete_tab": {
            const others = (await ctx.cache.list(args.spreadsheet_id)).titles.filter((t) => t.toLowerCase() !== info.title.toLowerCase());
            return {
        ...
        case "insert_columns":
        case "delete_columns": {
            const span = spanFrom(args.columns, "columns", 'Write it as "B:D" to mean columns B through D, or "C" for one column.');
            ...
            if (inserting && policy?.writableColumns?.length) {
                const shifted = policy.writableColumns.filter((entry) => /^[A-Za-z]{1,3}(:[A-Za-z]{1,3})?$/.test(entry.trim()) && shiftsPast(entry, span));
                if (shifted.length) {
                    warnings.push(`${describeSheet(policy)} names ${shifted.join(", ")} as writable by column letter, and this insert moves ${shifted.length === 1 ? "that column" : "those columns"} one letter to the right. Update ${policy.path} to match, or the reservation now covers a colleague's column.`);
                }
            }
            return {
                requests: [request],
        ...
        case "move_columns": {
            ...
            const to = requireString(args.to, "move_columns needs to.", 'Pass the column letter to put them before, for example "B".');
        ...
        case "sort": {
            if (!args.sort_by?.length) {
                throw err.invalid("sort needs sort_by.", 'Pass the columns to sort on, for example [{ "column": "Student" }, { "column": "Grade", "order": "desc" }].');
            }
            ...
            return {
                ...
                summary: `Sorted ${gridRangeToA1(range)} on "${info.title}" by ${listOf(args.sort_by.map((s) => `${s.column}${s.order === "desc" ? " descending" : ""}`))}.`,
                warnings: [
        ...
        case "dedupe": {
            ...
            return {
                ...
                gateRanges: [`${wholeTab}!${gridRangeToA1(range)}`],
                summary: `Removed duplicate rows from ${gridRangeToA1(range)} on "${info.title}"${args.key_columns?.length ? `, comparing ${listOf(args.key_columns)}` : ""}.`,
                warnings: [
        ...
        case "trim": {
            ...
            return {
                ...
                summary: `Trimmed leading and trailing whitespace in ${args.range?.trim() ?? "every cell"} on "${info.title}".`,
                warnings,
            };
        }
        case "group":
        ...
        case "protect": {
            const description = requireString(args.description, "protect needs a description.", "Say what the protection is for, in the words a colleague will read when Sheets warns them, for example \"Hadeer keeps the background check dates in K to P.\"");
            ...
            return {
                ...
                gateRanges: [],
                summary: `Protected ${args.range?.trim() ?? "the whole tab"} on "${info.title}"${warningOnly ? " with a warning" : " so only this account can edit it"}.`,
...
// ---------------------------------------------------------------------------
async function copyTabTo(ctx, args, info) {
    const destination = requireString(args.destination_spreadsheet_id, "copy_tab_to needs destination_spreadsheet_id.", "Pass the id of the spreadsheet to copy this tab into. It is the long id in the middle of that sheet's URL.");
    if (destination === args.spreadsheet_id) {
        throw err.invalid("destination_spreadsheet_id is this same spreadsheet.", "To copy a tab inside one spreadsheet use action duplicate_tab.");
    }
    const response = await withRetry(() => ctx.sheets.spreadsheets.sheets.copyTo({
    ...
    return ok(lines(`Copied "${info.title}" into spreadsheet ${destination} as "${newTitle}". The original is untouched.`, `\nWorth knowing:\n${warnings.map((w) => `- ${w}`).join("\n")}`), structured);
}
// ---------------------------------------------------------------------------
// Small pieces
// ---------------------------------------------------------------------------
function requireString(value, message, hint) {
    const v = value?.trim();
    if (!v)
        throw err.invalid(message, hint);
    return v;
}
function spanFrom(spec, what, hint) {
    if (!spec?.trim())
        throw err.invalid(`This action needs ${what}.`, hint);
    return what === "rows" ? parseRowSpan(spec) : parseColumnSpan(spec);
}
function dimensionRange(sheetId, dimension, span) {
    return { sheetId, dimension, startIndex: span.startIndex, endIndex: span.endIndex };
}
function outlineRange(args, info) {
    if (args.rows?.trim() && args.columns?.trim()) {
        throw err.invalid(`${args.action} takes rows or columns, not both.`, "A group covers one axis. Run the action twice to group both.");
    }
    if (args.rows?.trim()) {
 ...
 */
function dataRange(args, info, action) {
    if (args.range?.trim())
        return gridRange(info, args.range);
    const frozen = info.frozenRowCount ?? 0;
    if (frozen < 1) {
        throw err.invalid(`"${info.title}" has no frozen header row, so ${action} cannot tell where the data starts.`, `Pass range, for example A2:${columnIndexToLetter((info.columnCount ?? 26) - 1)}${info.rowCount ?? 1000}, or freeze the header first with sheets_style.`);
    }
    return {
        sheetId: info.sheetId,
...
async function headerRow(ctx, spreadsheetId, info) {
    ...
    const values = (response.data.valueRanges?.[0]?.values?.[0] ?? []);
    return values.map((v) => String(v ?? "").trim());
}
/** A column letter or a header name to its absolute index in the sheet. */
function resolveColumn(column, headers, info) {
    const wanted = column.trim();
    const byHeader = headers.findIndex((h) => h.toLowerCase() === wanted.toLowerCase());
    if (byHeader >= 0)
        return byHeader;
    if (/^[A-Za-z]{1,3}$/.test(wanted))
        return columnLetterToIndex(wanted);
    throw err.invalid(`No column named "${column}" on "${info.title}".`, headers.filter(Boolean).length
...
async function findProtection(ctx, args, info) {
    const response = await withRetry(() => ctx.sheets.spreadsheets.get({
        spreadsheetId: args.spreadsheet_id,
    ...
    const matches = all.filter((p) => {
        if (wantedRange && p.range.toLowerCase() !== wantedRange.toLowerCase())
            return false;
        if (wantedDescription && !p.description.toLowerCase().includes(wantedDescription))
            return false;
        return true;
    });
    if (matches.length === 1)
        return matches[0].id;
    if (matches.length === 0) {
        throw err.invalid(`No protection on "${info.title}" matches that.`, `The protections are: ${all.map((p) => `${p.range}${p.description ? ` (${p.description})` : ""}`).join("; ")}.`);
    }
    throw err.invalid(`${matches.length} protections on "${info.title}" match.`, `Narrow it with range or description. They are: ${matches
        .map((p) => `${p.range}${p.description ? ` (${p.description})` : ""}`)
        .join("; ")}.`);
}
/** Refuse a column delete that would take out a column the registry reserves. */
function guardWritableColumns(policy, span, action) {
    if (!policy?.writableColumns?.length)
        return;
    if (action !== "delete_columns" && action !== "insert_columns")
        return;
    const letters = [];
    for (let i = span.startIndex; i < span.endIndex; i += 1)
        letters.push(columnIndexToLetter(i));
    if (action === "delete_columns") {
        throw new GsheetsError("contract_violation", `Deleting ${listOf(letters)} is refused: this spreadsheet reserves columns for a colleague.`, `${describeSheet(policy)} lists ${policy.writableColumns.join(", ")} as ours. Deleting columns renumbers every column to the right of them, so it moves a colleague's columns whether or not it deletes them. Clear the values instead, or take the reservation out of the registry entry.`, { policy: policy.path, columns: letters });
    }
}
/** True when an insert at `span` renumbers the columns a registry entry names. */
function shiftsPast(entry, span) {
    try {
...
function describeReplies(replies, requests) {
    ...
    replies.forEach((reply, i) => {
        ...
        const dedupe = reply["deleteDuplicates"];
        if (dedupe)
            out["duplicates_removed"] = dedupe.duplicatesRemovedCount ?? 0;
        const trim = reply["trimWhitespace"];
        if (trim)
            out["cells_trimmed"] = trim.cellsChangedCount ?? 0;
        const protect = reply["addProtectedRange"];
