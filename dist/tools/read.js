 ...
 */
import { z } from "zod";
import { columnIndexToLetter, gridRangeToA1, parseA1, quoteSheetName, rangeCellCount, splitSheetRange, } from "../lib/a1.js";
import { colorStyleToText } from "../lib/colors.js";
import { describeNumberFormat } from "../lib/numfmt.js";
import { err } from "../lib/errors.js";
import { applyWhere, findInGrid, normalizeHeaders, paginate, toRecords, WHERE_OPERATORS, } from "../lib/records.js";
...
export const readInputSchema = {
    ...
    ranges: z
        ...
        .optional()
        .describe("Several ranges in one read, each optionally naming its own tab, for example [\"Roster!A1:C20\", \"Fees!A1:B9\"]. One values.batchGet, one block per range in the response, in the order given. Use this instead of one call per range: reads are capped at sixty a minute."),
    as: z
        .enum(["records", "grid"])
        .optional()
        .describe("records (default) keys each row by its header and stamps _row with the true sheet row. grid returns raw rows, which is what you want when there is no header."),
    values: z
        .enum(["formatted", "raw", "formulas", "both"])
        .optional()
        .describe("formatted (default) is what a person sees. raw is the underlying value. formulas shows =FORMULAS. both returns the displayed value and the formula side by side."),
    header_row: z
        .number()
        .int()
        .min(1)
...
export function createReadTool(deps) {
    return {
        ...
        config: {
            title: "Read a sheet",
            description: "Read values, formulas, or both. Returns records keyed by header with the true sheet row on each, or a raw grid. Pass ranges to read several blocks, across tabs if you like, in one call. Filter with where, search every tab with find, and page with limit and offset. Optionally returns cell formatting, notes and validation.",
        ...
        handler: guarded(async (raw) => {
            ...
            if (args.find)
                return runFind(ctx, args);
            if (args.ranges?.length)
                return runRanges(ctx, args);
            if (!args.sheet) {
                throw err.invalid("sheet is required.", "Pass the tab name, for example Tracker. To search every tab instead, pass find.");
            }
            ...
            if (rangeA1)
                parseA1(rangeA1); // Fail here, with our message, not the API's.
            const mode = args.values ?? "formatted";
            const primary = mode === "both" ? "formatted" : mode;
            const valueResponse = await withRetry(() => ctx.sheets.spreadsheets.values.batchGet({
            ...
            let formulaGrid;
            if (mode === "both") {
                const formulaResponse = await withRetry(() => ctx.sheets.spreadsheets.values.batchGet({
                    spreadsheetId: args.spreadsheet_id,
                    ranges: [reference],
                    valueRenderOption: "FORMULA",
                    majorDimension: "ROWS",
                }));
                formulaGrid = (formulaResponse.data.valueRanges?.[0]?.values ?? []);
            }
            ...
            if (asRecords && headerOffset !== undefined) {
                headers = normalizeHeaders((grid[headerOffset] ?? []).map((c) => (c === null || c === undefined ? "" : String(c))), Math.max(...grid.map((r) => r.length), 0));
            ...
            if (asRecords && headers.length) {
                ...
                if (args.columns?.length)
                    records = pickColumns(records, args.columns, headers);
            }
            else if (args.where?.length) {
                throw err.invalid("where needs records, and this read produced none.", "Either the tab has no header row or as was set to grid. Pass header_row to say which row holds the headers.");
            }
 ...
 */
export function resolveHeaderOffset(headerRow, originRow, frozenRowCount, grid, asRecords) {
    if (!asRecords)
        return undefined;
    if (headerRow !== undefined) {
        const offset = headerRow - originRow;
        if (offset < 0 || offset >= grid.length) {
            throw err.invalid(`Row ${headerRow} is not inside the range that was read.`, `The read starts at row ${originRow} and covers ${count(grid.length, "row")}. Widen the range, or pass a header_row inside it.`);
        }
        return offset;
    }
...
function attachFormulas(records, formulaRows, headers, firstDataRow) {
...
function pickColumns(records, wanted, headers) {
    const keys = wanted.map((name) => {
        ...
        throw err.invalid(`No column named "${name}".`, headers.length ? `The columns are: ${headers.join(", ")}.` : "This tab has no header row.");
    });
    return records.map((record) => {
        const out = { _row: record._row };
        for (const key of keys)
            out[key] = record[key];
        if ("_formulas" in record)
            out["_formulas"] = record["_formulas"];
        return out;
    });
}
async function readDetail(ctx, args, reference, originRow, originColumn) {
    const include = args.include;
    if (!include || (!include.formats && !include.notes && !include.validation))
        return { warnings: [] };
    ...
    rowData.forEach((row, r) => {
        (row.values ?? []).forEach((cell, c) => {
            ...
            if (include.formats && cell.userEnteredFormat) {
                ...
                if (numberFormat)
                    entry["number_format"] = numberFormat;
                if (f.horizontalAlignment)
                    entry["align"] = f.horizontalAlignment;
                if (f.wrapStrategy)
                    entry["wrap"] = f.wrapStrategy;
                if (f.textFormat?.bold)
                    entry["bold"] = true;
                if (f.textFormat?.italic)
                    entry["italic"] = true;
                if (f.textFormat?.fontSize)
                    entry["font_size"] = f.textFormat.fontSize;
                if (f.textFormat?.fontFamily)
                    entry["font"] = f.textFormat.fontFamily;
 ...
 */
async function runRanges(ctx, args) {
    const wanted = args.ranges ?? [];
    if (wanted.length > MAX_RANGES) {
        throw err.invalid(`${wanted.length} ranges is more than one read should carry.`, `Ask for at most ${MAX_RANGES} at a time. Past that the response is too big to be read carefully, and a whole tab is usually the better request.`);
    }
    ...
    const references = wanted.map((entry) => {
        ...
        if (!raw)
            throw err.invalid("One of the ranges is empty.");
        const split = splitSheetRange(raw);
        if (split.sheet)
            return raw;
        if (!args.sheet) {
            throw err.invalid(`"${raw}" does not say which tab it is on.`, "Either qualify it, as in Roster!A1:C20, or pass sheet to say which tab the unqualified ranges belong to.");
        }
        if (split.range)
            parseA1(split.range);
    ...
    const blocks = references.map((reference, index) => {
        const rows = (returned[index]?.values ?? []);
        return {
    ...
    return ok(lines(`${count(blocks.length, "range")} read in one call, values as ${mode}.`, ...blocks.map((b) => `  ${b.range}: ${count(b.rows, "row")} by ${count(b.columns, "column")}`), empty.length ? `\n${listOf(empty.map((r) => `"${r}"`))} came back empty.` : undefined), structured, { maxResultSizeChars: 150_000 });
}
async function runFind(ctx, args) {
    ...
    const wanted = find.in_sheets?.length
        ? find.in_sheets.map((name) => {
            ...
            if (!hit)
                throw err.sheetNotFound(name, all.titles);
            return hit;
        })
    ...
    (response.data.valueRanges ?? []).forEach((vr, index) => {
        ...
        const headers = normalizeHeaders((rows[0] ?? []).map((c) => (c === null || c === undefined ? "" : String(c))));
        hits.push(...findInGrid(sheet.title, rows, {
            query: find.query,
    ...
    const text = page.items.length
        ? lines(`Found "${find.query}" in ${count(page.total, "cell")} across ${listOf(wanted.map((s) => s.title))}.`, ...page.items.map((hit) => `  ${hit.sheet}!${hit.cell}${hit.header ? ` (${hit.header})` : ""}: ${hit.value}`), page.nextOffset ? `\nMore results. Read again with offset ${page.nextOffset}.` : "")
        : `No cell contains "${find.query}" on ${listOf(wanted.map((s) => s.title))}.`;
    return ok(text, structured, { maxResultSizeChars: 120_000 });
}
function readProse(sheet, range, mode, page, gridPage, filtered, warnings) {
    const head = page
        ? `${sheet} ${gridRangeToA1({})} read as ${count(page.total, "record")}${filtered ? `, ${filtered} filtered out` : ""}, showing ${page.items.length} from offset ${page.offset}.`
