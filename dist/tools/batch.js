...
import { resolveRequests, VALUE_CHANGING_REQUESTS, } from "../lib/rangeresolve.js";
...
export const batchInputSchema = {
    ...
    requests: z
        ...
        .min(1)
        .max(MAX_REQUESTS)
        .describe('Raw spreadsheets.batchUpdate requests, one request type per entry, for example { "sortRange": { "range": "\'Roster\'!A2:F80", "sortSpecs": [{ "column": 0 }] } }. Write sheet names where a sheetId belongs and A1 where a GridRange belongs; the server resolves both. Fields that want whole rows or columns take "5:9" or "B:D".'),
    sheet: z
        .string()
        .optional()
    ...
    dry_run: z
        .boolean()
        .optional()
        .describe("Resolve the requests and return the plan without sending anything. Worth doing once for any batch you have not run before."),
    force: z
        .boolean()
        .optional()
        .describe("Send even where the registry marks the spreadsheet read only. Read the refusal first: it names why somebody wrote that down."),
    check: z
        .boolean()
        .optional()
        .describe("Re-read the touched ranges afterwards and report formula errors. Default true. Turn it off only for a batch that touches no values."),
};
const DESCRIPTION = [
...
export function createBatchTool(deps) {
    return {
        name: "sheets_batch",
        config: {
        ...
        handler: guarded(async (raw) => {
            ...
            if (!Array.isArray(args.requests) || args.requests.length === 0) {
                throw err.invalid("requests is empty.", 'Pass at least one batchUpdate request, for example [{ "addChart": { "chart": { ... } } }].');
            }
            ...
            if (args.dry_run) {
                ...
                return ok(lines(`Dry run. ${count(resolved.requests.length, "request")} resolved, nothing sent.`, ...resolved.plan.map(planLine), resolved.touched.length
                    ? `\nTouches ${listOf(resolved.touched)}.`
                    : "\nNo request names a range, so the check would have nothing to re-read.", reminders.length ? `\nWorth knowing:\n${reminders.map((r) => `- ${r}`).join("\n")}` : undefined), structured, { maxResultSizeChars: 120_000 });
            }
            const result = await runBatchUpdate(ctx.sheets, spreadsheetId, resolved.requests);
            ...
            if (args.check !== false && changesValues && resolved.touched.length > 0) {
                check = await runErrorGate(ctx.sheets, spreadsheetId, resolved.touched);
            }
            const replies = (result.response?.replies ?? []);
            const warnings = [...reminders];
            if (resolved.unlocatable.length > 0 && changesValues) {
                warnings.push(`${listOf([...new Set(resolved.unlocatable)])} name no range, so the check below does not cover ${resolved.unlocatable.length === 1 ? "it" : "them"}.`);
            }
            const structured = {
                spreadsheet_id: spreadsheetId,
                dry_run: false,
            ...
            return ok(lines(`Applied ${count(resolved.requests.length, "request")} in one batchUpdate.`, ...resolved.plan.map(planLine), check ? `\n${describeCheck(check)}` : undefined, warnings.length ? `\nWorth knowing:\n${warnings.map((w) => `- ${w}`).join("\n")}` : undefined), structured, { maxResultSizeChars: 120_000 });
        }),
    };
}
 ...
 * it is not.
 */
function buildReminders(ctx, spreadsheetId, sheets, types) {
    const out = [];
    ...
    for (const sheet of sheets.length ? sheets : [undefined]) {
        ...
        if (policy.positionalRows)
            specifics.push("rows here are referred to by position elsewhere");
        if (policy.colleagueSafeText)
            specifics.push("text here is read by colleagues");
        out.push(`${describeSheet(policy)} marks this spreadsheet as ${policy.owner}${specifics.length ? ` and says ${listOf(specifics)}` : ""}. sheets_batch enforces none of that: it skips the formula guard, the writable column limits, the protection for dropdowns set by hand, and the refusal to reorder positional rows. Use sheets_write, sheets_style or sheets_structure unless the request type genuinely has no tool.`);
    }
    const rowMovers = types.filter((t) => ["deleteDimension", "insertDimension", "moveDimension", "sortRange", "deleteDuplicates", "randomizeRange"].includes(t));
    if (rowMovers.length && out.length) {
        out.push(`This batch includes ${listOf([...new Set(rowMovers)])}, which moves rows. sheets_structure would have refused that on a sheet marked positional_rows.`);
    }
    return out;
}
...
function compactReplies(replies) {
    return replies.flatMap((reply, index) => {
        ...
        if (body && typeof body === "object") {
            ...
            if (properties)
                entry["sheet"] = { sheet_id: properties.sheetId, title: properties.title };
        }
        return [entry];
    });
}
