...
import { buildCellFormat, describeRule, fingerprintRule, isMatch, resolveFingerprint, } from "../lib/cfrules.js";
...
import { FILL_ROLES, resolveFill } from "../lib/tablecolors.js";
const RULES_MASK = "sheets(properties(sheetId,title),conditionalFormats(ranges,booleanRule(condition(type,values(userEnteredValue,relativeDate)),format(backgroundColorStyle,backgroundColor,textFormat(bold,italic,strikethrough,underline,foregroundColorStyle,foregroundColor))),gradientRule(minpoint,midpoint,maxpoint)))";
const INTERPOLATION_TYPES = ["MIN", "MAX", "NUMBER", "PERCENT", "PERCENTILE"];
...
export const conditionalFormatInputSchema = {
    ...
    force: z.boolean().optional().describe("Act even where the registry marks the spreadsheet read only. Read the refusal first: it names why somebody wrote that down."),
};
export function createConditionalFormatTool(deps) {
    return {
        name: "sheets_conditional_format",
        config: {
            title: "Conditional formatting",
            description: "List, add, update and delete conditional format rules: boolean rules that paint when a condition holds, and gradient colour scales. Rules are addressed by a fingerprint of what they do rather than by index, because indexes shift under you. Fills can name a preset role such as ok, warn or flag.",
        ...
        handler: guarded(async (raw) => {
            ...
            if (args.sheet && !target) {
                throw err.sheetNotFound(args.sheet, all.map((s) => s.title));
            }
            if (args.action === "list")
                return listRules(args, all, target);
            if (!target) {
                throw err.invalid("sheet is required.", "Conditional format rules belong to one tab, so say which. Call action list with no sheet to see every tab's rules.");
            }
            ...
            if (args.action === "add")
                return addRule(ctx, args, target, preset);
            if (args.action === "update")
                return updateRule(ctx, args, target, preset);
...
// Actions
// ---------------------------------------------------------------------------
function listRules(args, all, target) {
    ...
    const body = sheets.map((sheet) => {
        ...
        const rows = sheet.rules.map((rule, index) => `  ${index}. ${describeRule(rule)}  [${fingerprintRule(rule)}]`);
        return lines(`\n${sheet.title}: ${count(sheet.rules.length, "rule")}, in evaluation order.`, ...rows);
    });
    return ok(lines("Rules are evaluated top down and the first match wins, so order matters.", ...body, "\nPass a fingerprint to update or delete one. It names what the rule does, so it still finds the rule after other rules move around it."), structured);
}
async function addRule(ctx, args, target, preset) {
    if (!args.ranges?.length) {
        throw err.invalid("ranges is required to add a rule.", 'Pass the A1 ranges the rule covers, for example ranges: ["E2:E200"].');
    }
    const rule = buildRule(args, target.sheetId, preset);
    const index = args.index ?? target.rules.length;
    const result = await runBatchUpdate(ctx.sheets, args.spreadsheet_id, [{ addConditionalFormatRule: { rule, index } }], { dryRun: args.dry_run === true });
    recordRuleRanges(args, target, rule);
    const fingerprint = fingerprintRule(rule);
    return ok(lines(`${args.dry_run ? "Would add" : "Added"} a rule at position ${index} on ${target.title}: ${describeRule(rule)}.`, `Fingerprint: ${fingerprint}`, index < target.rules.length
 ...
 * remembers.
 */
function recordRuleRanges(args, target, rule) {
    if (args.dry_run === true)
        return;
    for (const range of rule?.ranges ?? []) {
        recordWrite({
            spreadsheetId: args.spreadsheet_id,
            range: `${quoteSheetName(target.title)}!${gridRangeToA1(range)}`,
            sheet: target.title,
            tool: "sheets_conditional_format",
        });
    }
}
async function updateRule(ctx, args, target, preset) {
    const found = requireFingerprint(args, target);
    const merged = mergeRule(found.rule, args, target.sheetId, preset);
    const result = await runBatchUpdate(ctx.sheets, args.spreadsheet_id, [
        {
    ...
    recordRuleRanges(args, target, merged);
    return ok(lines(`${args.dry_run ? "Would update" : "Updated"} rule ${found.index} on ${target.title}.`, `Was: ${describeRule(found.rule)}`, `Now: ${describeRule(merged)}`, found.match === "moved"
        ...
        : undefined, `New fingerprint: ${fingerprintRule(merged)}`), {
        spreadsheet_id: args.spreadsheet_id,
        sheet: target.title,
        action: "update",
        applied: args.dry_run !== true,
...
async function deleteRule(ctx, args, target) {
    const found = requireFingerprint(args, target);
    const result = await runBatchUpdate(ctx.sheets, args.spreadsheet_id, [{ deleteConditionalFormatRule: { sheetId: target.sheetId, index: found.index } }], { dryRun: args.dry_run === true });
    recordRuleRanges(args, target, found.rule);
    return ok(lines(`${args.dry_run ? "Would delete" : "Deleted"} rule ${found.index} on ${target.title}: ${describeRule(found.rule)}.`, target.rules.length > found.index + 1
...
// Helpers
// ---------------------------------------------------------------------------
async function readRules(ctx, spreadsheetId) {
    const res = await withRetry(() => ctx.sheets.spreadsheets.get({ spreadsheetId, fields: RULES_MASK }));
...
function requireFingerprint(args, target) {
    if (!args.fingerprint?.trim()) {
        throw err.invalid(`fingerprint is required to ${args.action} a rule.`, "Call action list first and pass the fingerprint it printed beside the rule. Indexes are not accepted, because they shift whenever a rule is added or removed.");
    }
    ...
    throw new GsheetsError("invalid_argument", found.reason === "ambiguous"
        ? `More than one rule on ${target.title} matches that fingerprint.`
        : `No rule on ${target.title} matches the fingerprint ${args.fingerprint}.`, found.candidates.length
        ? `The rules on that tab now are:\n${listing}\nPick one of those fingerprints.`
        : "That tab has no conditional format rules at all. The rule was probably deleted, in the UI or by an earlier call.", { candidates: found.candidates });
}
function buildRule(args, sheetId, preset) {
    const ranges = (args.ranges ?? []).map((range) => {
        parseA1(range);
        return a1ToGridRange(range, sheetId);
    });
    const resolve = (token) => resolveFill(preset, token);
    if (args.kind === "gradient") {
        if (!args.gradient) {
            throw err.invalid("A gradient rule needs its points.", 'Pass gradient with min and max, and optionally mid, for example gradient: { min: { type: "MIN", color: "#FFFFFF" }, max: { type: "MAX", color: "ok" } }.');
        }
        const point = (p) => {
            const out = {
                type: p.type,
            ...
            if ((p.type === "NUMBER" || p.type === "PERCENT" || p.type === "PERCENTILE") && out.value === undefined) {
                throw err.invalid(`A ${p.type} gradient point needs a value.`, "MIN and MAX read the range's own extremes; the other three need the number to anchor on.");
            }
            return out;
        };
        const gradientRule = {
        ...
        if (args.gradient.mid)
            gradientRule["midpoint"] = point(args.gradient.mid);
        return { ranges, gradientRule };
    }
    if (!args.operator) {
        throw err.invalid("A boolean rule needs an operator.", 'For example operator: "equal" with value: "Overdue", or operator: "custom_formula" with a formula.');
    }
    ...
    if (!format) {
        throw err.invalid("A boolean rule needs a format, or it paints nothing.", 'Pass format, for example format: { fill: "flag" } or format: { bold: true, text_color: "#9B4A44" }.');
    }
 ...
 */
function mergeRule(existing, args, sheetId, preset) {
    const merged = {
    ...
    if (!condition) {
        throw err.invalid("That rule is a gradient, so an update has to say what it becomes.", "Pass gradient to change the colour scale, or operator and format to replace it with a boolean rule.");
    }
...
export function statusRuleFor(sheetId, rangeA1, option, fill, preset) {
    parseA1(rangeA1);
    return {
...
export function ruleRangeText(sheet, rule) {
