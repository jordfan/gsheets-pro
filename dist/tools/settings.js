 ...
 */
import { z } from "zod";
import { a1ToGridRange, columnIndexToLetter, gridRangeToA1, parseA1, quoteSheetName, toA1Reference, } from "../lib/a1.js";
...
import { isFormula, layoutSettingsBlock, SETTINGS_HEADERS, sanitizeNamedRange, } from "../lib/settings.js";
import { numberFormatForSettings, resolveFill, resolveHeaderText, resolveInputText, resolveMutedText, SETTINGS_FORMATS, } from "../lib/tablecolors.js";
import { metadataRequest } from "../lib/tables.js";
...
export function createSettingsTool(deps) {
    return {
        name: "sheets_settings",
        config: {
            title: "Settings block and named ranges",
            description: "Write a settings or assumptions block: one row per assumption with its label, value, unit and source, a named range on every value cell, the input colour on the value column, and a warning only protection over the structure. Also adds, repoints, renames and deletes named ranges on their own.",
        ...
        handler: guarded(async (raw) => {
            ...
            const state = await withRetry(() => ctx.sheets.spreadsheets.get({ spreadsheetId: args.spreadsheet_id, fields: NAMED_RANGE_MASK }));
            const named = (state.data.namedRanges ?? []).flatMap((n) => n.namedRangeId && n.name
                ? [{ namedRangeId: n.namedRangeId, name: n.name, range: (n.range ?? undefined) }]
                : []);
            const titleBySheetId = new Map((state.data.sheets ?? []).flatMap((s) => s.properties?.sheetId !== undefined && s.properties?.sheetId !== null && s.properties.title
                ? [[s.properties.sheetId, s.properties.title]]
                : []));
            if (action === "list_named_ranges")
                return listNamedRanges(args, named, titleBySheetId);
            if (action !== "block")
                return manageNamedRange(ctx, args, action, named, titleBySheetId);
...
// ---------------------------------------------------------------------------
function listNamedRanges(args, named, titles) {
    const rows = named
        .map((n) => ({
        name: n.name,
    ...
    return ok(rows.length
        ? lines(`${count(rows.length, "named range")} in this spreadsheet.`, ...rows.map((r) => `  ${r.name} -> ${r.range}`))
        : "This spreadsheet has no named ranges. A formula that reads Fee_per_session instead of $B$4 is worth the two seconds it takes to make one.", { spreadsheet_id: args.spreadsheet_id, named_ranges: rows });
}
async function manageNamedRange(ctx, args, action, named, titles) {
    const wanted = String(args.name ?? "").trim();
    if (!wanted) {
        throw err.invalid("name is required.", "Say which named range, for example name: Fee_per_session. Call action list_named_ranges to see them.");
    }
    const existing = named.find((n) => n.name.toLowerCase() === wanted.toLowerCase());
    if (action === "delete_named_range") {
        if (!existing) {
            throw new GsheetsError("not_found", `No named range called "${wanted}".`, named.length
                ? `The named ranges are: ${named.map((n) => n.name).join(", ")}.`
                : "This spreadsheet has no named ranges.");
        }
        const result = await runBatchUpdate(ctx.sheets, args.spreadsheet_id, [{ deleteNamedRange: { namedRangeId: existing.namedRangeId } }], { dryRun: args.dry_run === true });
        return ok(lines(`${args.dry_run ? "Would delete" : "Deleted"} the named range ${existing.name}.`, "Any formula that referred to it now reads #NAME?, so check the ones that did."), {
    ...
    if (action === "add_named_range") {
        if (existing) {
            throw new GsheetsError("invalid_argument", `A named range called "${existing.name}" already exists.`, `It points at ${existing.range ? toA1Reference(titles.get(existing.range.sheetId ?? -1) ?? "?", gridRangeToA1(existing.range)) : "an unknown range"}. Use action update_named_range to repoint it, or pick another name.`);
        }
        if (!rangeA1) {
        ...
        const result = await runBatchUpdate(ctx.sheets, args.spreadsheet_id, [{ addNamedRange: { namedRange: { name, range: a1ToGridRange(rangeA1, info.sheetId) } } }], { dryRun: args.dry_run === true });
        return ok(lines(`${args.dry_run ? "Would name" : "Named"} ${toA1Reference(info.title, rangeA1)} as ${name}.`, name === wanted ? undefined : `"${wanted}" is not a name Sheets accepts, so it became ${name}.`), {
            spreadsheet_id: args.spreadsheet_id,
            action,
            applied: args.dry_run !== true,
            dry_run: args.dry_run === true,
    ...
    if (!existing) {
        throw new GsheetsError("not_found", `No named range called "${wanted}".`, "Use action add_named_range to make one, or list_named_ranges to see what exists.");
    }
    const namedRange = { namedRangeId: existing.namedRangeId };
    const fields = [];
    if (rangeA1) {
        parseA1(rangeA1);
    ...
    if (args.new_name?.trim()) {
        namedRange["name"] = sanitizeNamedRange(args.new_name, new Set(named.filter((n) => n.namedRangeId !== existing.namedRangeId).map((n) => n.name)));
        fields.push("name");
    }
    if (fields.length === 0) {
        throw err.invalid("Nothing to change.", "Pass range to repoint the named range, new_name to rename it, or both.");
    }
    const result = await runBatchUpdate(ctx.sheets, args.spreadsheet_id, [{ updateNamedRange: { namedRange, fields: fields.join(",") } }], { dryRun: args.dry_run === true });
    return ok(`${args.dry_run ? "Would update" : "Updated"} the named range ${existing.name}${namedRange["name"] ? ` (now ${namedRange["name"]})` : ""}${rangeA1 ? ` to cover ${toA1Reference(info.title, rangeA1)}` : ""}.`, {
        spreadsheet_id: args.spreadsheet_id,
        action,
        applied: args.dry_run !== true,
        dry_run: args.dry_run === true,
        name: namedRange["name"] ?? existing.name,
...
// The block
// ---------------------------------------------------------------------------
async function writeBlock(ctx, args, named) {
    if (!args.sheet) {
        throw err.invalid("sheet is required.", "Say which tab the settings block goes on.");
    }
    const items = args.items ?? [];
    if (!items.length) {
        throw err.invalid("items is required for a settings block.", 'Each item is one assumption: { label: "Fee per session", value: 55, unit: "dollars", source: "2026-27 vendor agreement" }.');
    }
    const info = await ctx.cache.resolve(args.spreadsheet_id, args.sheet);
    ...
    for (const entry of named) {
        if (insideBlock(entry.range))
            reusable.set(entry.name.toLowerCase(), entry);
        else
        ...
        requests.push({
            addProtectedRange: {
                protectedRange: {
                    range: blockRange,
                    warningOnly: true,
                    description: "Settings block. Change the values; leave the labels, units and sources where they are, because formulas and named ranges point at these cells.",
                },
            },
        });
    }
    ...
    const record = {
        ...(sheetRecord(snapshot, info.sheetId) ?? {}),
        preset: preset.name,
    ...
    const check = args.dry_run === true
        ? undefined
        : await runErrorGate(ctx.sheets, args.spreadsheet_id, [reference]);
    const structured = {
        spreadsheet_id: args.spreadsheet_id,
        sheet: info.title,
        action: "block",
    ...
    return ok(lines(`${args.dry_run ? "Would write" : "Wrote"} a settings block on ${info.title} at ${layout.blockA1}: ${count(items.length, "assumption")}.`, namedRangeReport.length
        ...
        : undefined, `Values are the ${preset.name} preset's input colour, so it is visible which cells a person is meant to type in.`, args.protect !== false
        ? "The block carries a warning only protection: an edit asks first, and nobody is locked out."
        : undefined, check ? describeCheck(check) : "Dry run: nothing was sent."), structured);
}
/** The updateCells requests that paint and fill the block. */
function cellRequests(layout, args, preset, sheetId, startColumn) {
    const requests = [];
                        ...
                        values: [
                            {
                                ...
                                userEnteredFormat: {
                                    textFormat: {
                                        ...
                                        fontFamily: title?.font === "display" ? (preset.fonts.display ?? preset.fonts.primary) : preset.fonts.primary,
...
function extendedValue(value, format) {
    ...
    if (format && format !== "text") {
        ...
        if (Number.isFinite(n))
            return { numberValue: format === "percent" && text.includes("%") ? n / 100 : n };
    }
