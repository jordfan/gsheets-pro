...
export function exportUrl(spreadsheetId, options) {
 ...
 */
export async function fetchExportPdf(url, accessToken, fetchImpl = fetch) {
    const response = await fetchImpl(url, { headers: { Authorization: `Bearer ${accessToken}` } });
    const contentType = response.headers.get("content-type") ?? undefined;
    const bytes = Buffer.from(await response.arrayBuffer());
    if (!response.ok) {
        throw new GsheetsError(response.status === 401 || response.status === 403 ? "permission_denied" : "unavailable", `The export endpoint answered ${response.status}${contentType ? ` with ${contentType}` : ""}.`, response.status === 401 || response.status === 403
    ...
    if (bytes.subarray(0, 5).toString("latin1") !== "%PDF-") {
        throw new GsheetsError("unavailable", `The export endpoint returned ${contentType ?? "something"} rather than a PDF (${bytes.length} bytes).`, "This is usually a sign-in page, which means the credential was not accepted. Run `gsheets-pro doctor`.");
    }
    if (bytes.length > MAX_PDF_BYTES) {
        throw new GsheetsError("result_too_large", `The rendered PDF is ${Math.round(bytes.length / 1024 / 1024)} MB, over the ${Math.round(MAX_PDF_BYTES / 1024 / 1024)} MB cap.`, "Render one tab at a time, or pass a range covering the part worth looking at.");
    }
...
export const PDFTOPPM_HINT = "Rendering needs pdftoppm from poppler. Install it with `brew install poppler` on macOS or `apt install poppler-utils` on Debian and Ubuntu. Everything except sheets_render works without it.";
export function pdftoppmBinary(env = process.env) {
    return env.GSHEETS_PRO_PDFTOPPM?.trim() || "pdftoppm";
}
/** Is poppler here? Cheap enough to ask before writing a PDF to disk. */
export async function hasPdftoppm(binary = pdftoppmBinary(), run = defaultRun) {
    try {
        await run(binary, ["-v"]);
        return true;
    }
 ...
 */
export async function pdfToPng(pdfPath, prefix, options = {}) {
    ...
    if (paths.length === 0) {
        throw new GsheetsError("unavailable", "pdftoppm produced no images from the exported PDF.", "The PDF may be empty, which happens when the tab has no content in the range asked for.");
    }
...
export function serializeRender(work) {
...
export function renderFileName(sheetTitle, at = Date.now()) {
