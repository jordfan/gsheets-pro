 ...
 */
export function renderSecret(env = process.env) {
...
export function resetRenderSecret() {
    ephemeralSecret = undefined;
}
export function signRender(name, expiresAt, secret) {
    return crypto.createHmac("sha256", secret).update(`${name}.${expiresAt}`).digest("hex");
}
export function signRenderPath(name, options = {}) {
 ...
 */
export function verifyRenderRequest(name, params, options = {}) {
    const decoded = safeDecode(name);
    if (!decoded || !RENDER_NAME_PATTERN.test(decoded))
...
//# sourceMappingURL=rendersign.js.map