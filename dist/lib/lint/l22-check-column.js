    ...
    run(ctx) {
        ...
        for (const column of columns) {
            ...
            findings.push({
                rule: "L22",
                severity: "warning",
                location: lintLocation(ctx.title, `${column.letter}${Math.min(...flagged.map((f) => f.row)) + 1}:${column.letter}${Math.max(...flagged.map((f) => f.row)) + 1}`),
                message: `The Check column ${named} on ${ctx.title} is flagging ${flagged.length} row(s): ${quoted}${rest > 0 ? `; and ${rest} more` : ""}. The spreadsheet is telling you about itself.`,
...
function displayedText(ctx, row, column) {
    for (const block of ctx.sheet.data ?? []) {
        ...
        if (value && typeof value.numberValue === "number")
...
//# sourceMappingURL=l22-check-column.js.map