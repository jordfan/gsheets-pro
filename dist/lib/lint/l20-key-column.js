...
import { cellA1, columnsWithRole, dataRegion, gridValue, lintLocation, valueGrid, } from "./types.js";
/** How many offending rows one finding names before it summarises. */
export const MAX_KEY_ROWS_NAMED = 10;
export const l20KeyColumn = {
    id: "L20",
    severity: "warning",
    ...
    run(ctx) {
        ...
        if (blanks.length === 0 && duplicates.length === 0)
            return [];
...
function keyColumn(ctx) {
    const contract = ctx.contract;
    if (!contract)
        return undefined;
    const roleColumns = columnsWithRole(contract, "key");
    if (roleColumns.length > 0)
        return roleColumns[0];
    const named = contract.keyColumn?.trim();
    if (!named)
        return undefined;
    // The metadata may name the column by letter or by header text.
    const byHeader = contract.columns.find((column) => column.header && column.header.trim().toLowerCase() === named.toLowerCase());
    if (byHeader) {
        const out = { index: byHeader.index, letter: byHeader.letter };
        if (byHeader.header)
            out.header = byHeader.header;
        return out;
    }
    try {
        const index = columnLetterToIndex(named);
        const column = contract.columns.find((c) => c.index === index);
        const out = { index, letter: named.toUpperCase() };
        if (column?.header)
...
function displayValue(grid, column, rows) {
    const first = rows[0];
    return gridValue(grid, first, column).trim();
}
//# sourceMappingURL=l20-key-column.js.map