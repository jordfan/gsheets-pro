...
export function resolveNumberFormat(spec) {
    ...
    if (raw === "@" || /@/.test(withoutLiterals))
        return { type: "TEXT", pattern: raw };
    if (hasDate && hasClock)
        return { type: "DATE_TIME", pattern: raw };
    if (hasDate)
        return { type: "DATE", pattern: raw };
    if (hasClock)
        return { type: "TIME", pattern: raw };
    if (/%/.test(withoutLiterals))
        return { type: "PERCENT", pattern: raw };
    if (/[$£€¥]/.test(raw))
        return { type: "CURRENCY", pattern: raw };
 ...
 */
export function describeNumberFormat(format) {
    if (!format)
        return undefined;
    ...
    for (const [name, known] of Object.entries(NUMBER_FORMAT_SHORTHAND)) {
        if (known.pattern === pattern && known.type === type)
            return name;
    }
