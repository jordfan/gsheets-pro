...
import { parseColorStyle, THEME_SLOTS } from "./colors.js";
...
export const FILL_ROLES = ["header", "band1", "band2", "ok", "warn", "flag", "muted"];
/** The four roles that stand in for the chip colors the API cannot set. */
export const STATUS_ROLES = ["ok", "warn", "flag", "muted"];
// ---------------------------------------------------------------------------
// Loading
// ---------------------------------------------------------------------------
const cache = new Map();
...
export function parsePreset(raw, name) {
    const preset = raw;
    const missing = [];
    if (!preset || typeof preset !== "object")
        missing.push("the whole file");
    if (!preset?.fonts?.primary)
        missing.push("fonts.primary");
    if (!preset?.roles?.header?.fill || !preset?.roles?.header?.text)
        missing.push("roles.header");
    for (const role of ["band1", "band2", "ok", "warn", "flag", "muted", "muted_fill"]) {
        if (!preset?.roles?.[role])
            missing.push(`roles.${role}`);
    }
    for (const slot of THEME_SLOTS) {
        if (!preset?.theme?.[slot])
            missing.push(`theme.${slot}`);
    }
    for (const key of ["currency", "percent", "date", "integer"]) {
        if (!preset?.numbers?.[key])
            missing.push(`numbers.${key}`);
    }
    if (missing.length) {
        throw new GsheetsError("invalid_argument", `The preset "${name}" is missing ${missing.join(", ")}.`, "A preset must match presets/schema.json: nine theme slots, the header and band roles, the four status roles, and the currency, percent, date and integer number formats.");
    }
    return { ...preset, name: preset.name ?? name };
}
/** Load a preset by name, cached at module scope. */
export function loadPreset(name = DEFAULT_PRESET) {
    ...
    if (!fs.existsSync(file)) {
        ...
        throw new GsheetsError("invalid_argument", `No preset named "${wanted}".`, names.length
            ? `The presets on this host are: ${names.join(", ")}.`
            : "No preset files were found. Set GSHEETS_PRO_PRESETS to the directory holding them.", { available: names });
    }
    let raw;
    try {
        ...
        throw new GsheetsError("invalid_argument", `The preset "${wanted}" is not valid JSON: ${error.message}`, "Fix the file, or pass a different preset.");
    }
...
// Resolving a color
// ---------------------------------------------------------------------------
function roleValue(preset, role) {
    switch (role) {
        case "header":
 ...
 */
export function resolveFill(preset, spec) {
    ...
    try {
        ...
        throw new GsheetsError("invalid_argument", error.message, fillHint(preset));
    }
}
export function fillHint(preset) {
...
export function resolveMutedText(preset) {
 ...
 */
export function tableRowsProperties(preset) {
    return {
...
export function numberFormatForColumnType(preset, columnType) {
    switch (columnType) {
        case "CURRENCY":
...
export function numberFormatForSettings(preset, format) {
    switch (format) {
        case "currency":
            return { type: "CURRENCY", pattern: preset.numbers.currency };
        case "percent":
...
//# sourceMappingURL=tablecolors.js.map