 ...
 */
export const DEFAULT_LEAF_FIELDS = [
    "numberFormat",
    "padding",
    "textRotation",
    "textFormat.link",
...
function isLeafField(name, path, leaves) {
    if (leaves.has(name) || leaves.has(path))
        return true;
 ...
 */
export function fieldMaskPaths(value, options = {}) {
    const leaves = new Set(options.leaves ?? DEFAULT_LEAVES);
...
export function buildFieldMask(value, options = {}) {
...
export function isEmptyRequest(value) {
