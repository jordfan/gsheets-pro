...
import { z } from "zod";
import { columnLetterToIndex, parseColumnSpan } from "./a1.js";
...
export function findRegistryPath(startDir = process.cwd()) {
...
export function loadRegistry(options = {}) {
    ...
    if (!fs.existsSync(file)) {
        if (options.file || fromEnv) {
            throw new GsheetsError("invalid_argument", `No registry file at ${file}.`, "Point GSHEETS_PRO_REGISTRY at a readable file, or remove it to fall back to the repo's .claude/gsheets-pro.json.");
        }
        return undefined;
    }
...
export function parseRegistry(text, file = REGISTRY_RELATIVE_PATH) {
    let raw;
    try {
        raw = JSON.parse(text);
    }
    catch (error) {
        throw new GsheetsError("invalid_argument", `${file} is not valid JSON: ${error.message}`, "Fix the file, or delete it. A registry that cannot be read is treated as a hard error, because it is what protects shared sheets from being written to.");
    }
    ...
    if (!parsed.success) {
        const issues = parsed.error.issues
            ...
            .join("; ");
        throw new GsheetsError("invalid_argument", `${file} does not match the registry schema: ${issues}`, 'Each entry under "spreadsheets" is keyed by spreadsheet id and may set owner, writable_columns, positional_rows, colleague_safe_text, allowlist, note, and per tab overrides under "sheets".');
    }
    return buildRegistry(parsed.data, file);
}
function buildRegistry(fileData, filePath) {
    const ids = Object.keys(fileData.spreadsheets ?? {});
    return {
        path: filePath,
        file: fileData,
        ids,
        policyFor(spreadsheetId, sheet) {
            const entry = fileData.spreadsheets?.[spreadsheetId];
            if (!entry)
                return undefined;
            const tabPolicy = sheet ? findTabPolicy(entry, sheet) : undefined;
            const layered = [fileData.defaults, entry, tabPolicy].filter(Boolean);
            const pick = (key) => {
            ...
            if (writable)
                policy.writableColumns = writable;
...
function findTabPolicy(entry, sheet) {
    if (!entry.sheets)
        return undefined;
...
function resolveWritable(entries) {
    const spans = [];
    const headers = [];
 ...
 */
export function isColumnWritable(policy, column) {
...
export function describeSheet(policy) {
    if (policy.name && policy.sheet)
        return `${policy.name}, tab ${policy.sheet},`;
    if (policy.name)
        return `The registry entry for ${policy.name}`;
    return "The registry";
}
/** A one line summary for `sheets_open`. */
export function describePolicy(policy) {
    const parts = [`owner: ${policy.owner}`];
    if (policy.writableColumns?.length) {
        parts.push(`writable columns: ${policy.writableColumns.join(", ")}`);
    }
    if (policy.positionalRows)
        parts.push("rows are positional, so never sort, insert, or delete");
    if (policy.colleagueSafeText)
        parts.push("text here is read by colleagues");
    if (policy.note)
        parts.push(policy.note);
    return parts.join("; ");
}
/** One sentence saying this sheet is not ours, for a refusal message. */
export function readOnlyReason(policy) {
    return `${describeSheet(policy)} marks this spreadsheet read only, so nothing here is ours to change.${policy.note ? ` ${policy.note}` : ""}`;
}
/**
 ...
 */
export function assertWritable(policy, options) {
    if (!policy?.readOnly || options.force === true)
        return;
    throw new GsheetsError("contract_violation", `${options.tool} was called on a spreadsheet the registry marks read only. ${readOnlyReason(policy)}`, "Nothing on this sheet is ours to change, whatever its columns say. If the registry is out of date, fix the registry rather than passing force; if a person has genuinely asked for this one change, force is how you say so.", { read_only: true, registry: policy.path });
}
/** True when a structure change that moves rows is forbidden here. */
export function refusesRowMoves(policy) {
