...
export function setRenderMode(mode, options = {}) {
    const next = { mode };
    if (options.dir)
        next.dir = options.dir;
    if (options.publicUrl)
        next.publicUrl = options.publicUrl;
    config = next;
}
/** Forget the configured mode. Tests, and `doctor`. */
export function resetRenderMode() {
    config = undefined;
}
export function renderMode(env = process.env) {
    const override = env.GSHEETS_PRO_RENDER_MODE?.trim().toLowerCase();
    if (override === "hosted" || override === "local")
        return override;
 ...
 */
export function renderDir(env = process.env) {
...
export function renderPublicUrl(env = process.env) {
