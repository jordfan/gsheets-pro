...
import { contrastRatio, parseColor, parseThemeSlot, THEME_SLOTS, } from "./colors.js";
import { GsheetsError } from "./errors.js";
import { resolveNumberFormat } from "./numfmt.js";
...
export const MODEL_ONLY_ROLES = ["input", "formula", "cross_sheet"];
// ---------------------------------------------------------------------------
 ...
 */
export function presetSearchPaths(options = {}) {
    const out = [];
    const fromEnv = process.env.GSHEETS_PRO_PRESETS;
    if (fromEnv)
        out.push(...fromEnv.split(path.delimiter).filter(Boolean));
    out.push(path.join(dataDir(), "presets"));
    const pluginRoot = process.env.CLAUDE_PLUGIN_ROOT;
    if (pluginRoot)
        out.push(path.join(pluginRoot, "presets"));
    for (const dir of bundledPresetDirs(options.cwd))
        out.push(dir);
    return [...new Set(out)];
}
/**
 ...
 */
function bundledPresetDirs(cwd) {
    const out = [];
...
export function parsePreset(text, source = "<inline>") {
    let raw;
    try {
        raw = JSON.parse(text);
    }
    catch (error) {
        throw new GsheetsError("invalid_argument", `${source} is not valid JSON: ${error.message}`, "A preset is a JSON file shaped like presets/schema.json. Copy presets/neutral.json and change the values.");
    }
    ...
    if (!parsed.success) {
        const issues = parsed.error.issues
            ...
            .join("; ");
        throw new GsheetsError("invalid_argument", `${source} does not match the preset schema: ${issues}`, "Every preset needs name, fonts, all nine theme slots, the roles header, band1, band2, ok, warn, flag and muted, and the numbers currency, percent, date and integer. There is no footer role and there cannot be one.");
    }
    return parsed.data;
}
/** Read a named preset off disk, searching the preset paths in order. */
export function loadPreset(name, options = {}) {
    const wanted = String(name ?? "").trim().toLowerCase();
    if (!wanted)
        throw new GsheetsError("invalid_argument", "No preset name given.");
    ...
    throw new GsheetsError("invalid_argument", `No preset named "${name}".`, `Available presets: ${listPresets(options).join(", ") || "none found"}. Presets are read from ${dirs.join(", ")}.`, { searched: dirs });
}
...
export function resolveToken(preset, token) {
    const slot = parseThemeSlot(token);
    if (slot)
        return parseColor(preset.theme[slot]);
...
export function checkPreset(preset) {
    ...
    for (const [name, spec] of Object.entries(preset.numbers)) {
        ...
        try {
            ...
            problems.push({
                severity: "error",
                message: `The ${name} number format is not usable: ${error.message}`,
            });
        }
    }
    return problems;
...
export function validatePreset(preset) {
    ...
    if (errors.length) {
        throw new GsheetsError("invalid_argument", `The preset "${preset.name}" is not usable: ${errors.map((e) => e.message).join(" ")}`, "Fix the preset file rather than working around it here. A preset that fails these checks produces a sheet that is hard to read, which is the one thing this plugin exists to prevent.");
    }
 ...
 */
export function resolvePresetName(sources = {}) {
    const clean = (v) => {
...
export function readDataDirDefaults() {
    ...
    try {
        ...
        if (parsed.data.preset)
            out.preset = parsed.data.preset;
        if (parsed.data.archetype)
            out.archetype = parsed.data.archetype;
        return out;
    }
    catch {
...
export function resolveArchetype(preset, sources = {}) {
...
export function tokenToColorStyle(token) {
    const slot = parseThemeSlot(token);
    if (slot)
 ...
 */
export function compileTheme(preset, options = {}) {
    ...
    const themeColors = THEME_SLOTS.map((slot) => {
        ...
        if (fromPreset)
            return { colorType: slot, color: { rgbColor: parseColor(fromPreset) } };
        const inherited = currentBySlot.get(slot);
        if (inherited) {
            inheritedSlots.push(slot);
            return { colorType: slot, color: { rgbColor: inherited } };
        }
        throw new GsheetsError("invalid_argument", `The theme slot ${slot} has no value in preset "${preset.name}" and the spreadsheet has none to inherit.`, "A theme write must carry all nine slots. Add the slot to the preset.");
    });
    ...
    if (preset.roles.title) {
        ...
        if (title.size !== undefined)
            textFormat.fontSize = title.size;
        if (title.bold !== undefined)
            textFormat.bold = title.bold;
        if (title.text)
            textFormat.foregroundColorStyle = tokenToColorStyle(title.text);
    ...
    for (const role of ["ok", "warn", "flag"]) {
    ...
    if (archetype === "model") {
        for (const role of MODEL_ONLY_ROLES) {
            const token = preset.roles[role];
            if (token)
                roles[role] = { textFormat: { foregroundColorStyle: tokenToColorStyle(token) } };
        }
    }
    const numberFormats = {};
    for (const [name, spec] of Object.entries(preset.numbers)) {
        if (typeof spec === "string")
            numberFormats[name] = resolveNumberFormat(spec);
    }
    const tabColors = {};
    for (const role of ["inputs", "workings", "outputs"]) {
        const token = preset.tabs?.[role];
        if (token)
            tabColors[role] = tokenToColorStyle(token);
    }
    return {
        preset,
        archetype,
 ...
 */
export function roleFormat(compiled, role) {
    const wanted = String(role ?? "").trim().toLowerCase();
    if (!ROLE_TOKENS.includes(wanted)) {
        throw new GsheetsError("invalid_argument", `There is no style role named "${role}".`, `The roles are: ${ROLE_TOKENS.join(", ")}. There is no footer role, because a Table's footer color overwrites the last data row with SUM formulas.`);
    }
    const format = compiled.roles[wanted];
    if (format)
        return format;
    if (MODEL_ONLY_ROLES.includes(wanted)) {
        throw new GsheetsError("invalid_argument", `The role "${wanted}" belongs to the model archetype, and this sheet is a ${compiled.archetype}.`, "On a tracker, font color carries no meaning and state lives in a Status column painted by a dropdown or a conditional rule. If this really is a model, pass archetype: \"model\".");
    }
    throw new GsheetsError("invalid_argument", `The preset "${compiled.preset.name}" defines no ${wanted} role.`);
}
/** The manifest metadata value a theme write records on the spreadsheet. */
export function manifestValue(compiled, now = new Date()) {
