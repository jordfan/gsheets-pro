 ...
 */
import { a1ToGridRange, columnIndexToLetter, gridRangeToA1, parseColumnSpan, parseRowSpan, quoteSheetName, splitSheetRange, } from "./a1.js";
 ...
 */
export const DIMENSION_RANGE_PATHS = [
...
export const RANGE_FIELD_NAMES = [
    "range",
    "ranges",
    "gridRange",
    "sourceRange",
...
export const VALUE_CHANGING_REQUESTS = [
    "appendCells",
    "autoFill",
    "copyPaste",
    "cutPaste",
...
export function describeRequestType(type) {
...
export function qualifiedA1(title, range) {
    const body = gridRangeToA1(range);
    return body === "the whole sheet" ? quoteSheetName(title) : `${quoteSheetName(title)}!${body}`;
}
function dimensionToA1(title, dimension, start, end) {
    const body = dimension === "COLUMNS"
 ...
 */
export async function resolveRequests(rawRequests, options) {
    const requests = [];
    const plan = [];
    const touched = [];
    const sheets = [];
    const unlocatable = [];
    const noteSheet = (title) => {
        if (title && !sheets.includes(title))
            sheets.push(title);
    };
    const noteRange = (reference) => {
        if (reference && !touched.includes(reference))
            touched.push(reference);
    };
    for (const [index, raw] of rawRequests.entries()) {
        if (!isPlainObject(raw)) {
            throw err.invalid(`requests[${index}] is not an object.`, 'Each entry is one batchUpdate request keyed by its type, for example { "sortRange": { "range": "\'Roster\'!A2:F80", "sortSpecs": [...] } }.');
        }
        const keys = Object.keys(raw);
        if (keys.length !== 1) {
            throw err.invalid(`requests[${index}] has ${keys.length} keys (${keys.join(", ") || "none"}), and a batchUpdate request has exactly one.`, "Split it into one entry per request type. The API reads the single key as the request type.");
        }
        const type = keys[0];
        const targets = [];
...
async function walk(node, path, ctx) {
    if (Array.isArray(node)) {
        const out = [];
        for (const item of node)
            out.push(await walk(item, path, ctx));
        return out;
    }
    ...
    for (const [key, value] of Object.entries(node)) {
        ...
        if (Array.isArray(value) && RANGE_FIELDS.has(key) && value.every((v) => typeof v === "string")) {
            ...
            for (const item of value)
                converted.push(await stringToRange(item, childPath, ctx));
            out[key] = converted;
            continue;
        }
 ...
 */
async function report(node, path, sheetId, ctx) {
    if (sheetId === undefined)
        return;
    const hasGridBounds = "startRowIndex" in node ||
        "endRowIndex" in node ||
...
async function stringToRange(value, path, ctx) {
    ...
    if (!sheetName) {
        throw err.invalid(`The range "${value}" at ${path} does not say which tab it is on.`, "Qualify it, for example 'Roster'!A2:F80, or pass sheet so unqualified ranges land on that tab.");
    }
    const info = await ctx.options.resolveSheet(sheetName);
    if (wantsDimension) {
        const body = split.range.trim();
        if (!body) {
            throw err.invalid(`${path} needs a band of rows or columns and "${value}" names a whole tab.`, 'Write it as rows ("5:9") or columns ("B:D").');
        }
...
function toSpan(body, original, path) {
    ...
    throw new GsheetsError("bad_range", `${path} wants whole rows or whole columns, and "${original}" names a block of cells.`, 'Write columns as "B:D" or rows as "5:9". A block like A2:F80 belongs on a field that takes a GridRange, such as sortRange.range.');
}
//# sourceMappingURL=rangeresolve.js.map