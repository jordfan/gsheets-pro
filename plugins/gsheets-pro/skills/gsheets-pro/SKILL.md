...
The exception is **dropdowns, which never render as chips**. A validation rule
created through the API shows as plain cell text, with no pill and no arrow,
whether it came from a Table column typed `DROPDOWN` or from
`sheets_validation`. So a missing chip in a render means nothing at all.
`sheets_check` is authoritative for validation state. Confirm a dropdown there,
and never re-create one because the picture looked bare.
