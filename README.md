...
| Piece | State |
|---|---|
| Core library, both transports, `sheets_open`, `sheets_read` | working, live tested |
| Skill, card, references, presets, hooks, reviewer agent | written, validated |
| `sheets_write`, `sheets_style`, presets compiler | working, live tested |
| `sheets_table`, `sheets_settings`, `sheets_validation`, `sheets_conditional_format` | working, live tested |
| `sheets_structure`, `sheets_find`, `sheets_batch` | working, live tested |
| `sheets_check`, `sheets_render`, the seven v1 lint rules | working, live tested |
| Dockerfile, hosting guide, `vendor` for cloud sessions | working |
| Evals, setup walkthrough, first release | in progress |

801 offline tests run in about a second and need no credentials. A separate
